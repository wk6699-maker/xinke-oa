...
const storagePreference = String(process.env.STORAGE_MODE || 'sqlite').toLowerCase();
...
const sqlitePath = resolve(projectRoot, process.env.SQLITE_PATH || './data/xinke-email.sqlite');
const allowSqliteFallback = String(process.env.LOCAL_SQLITE_FALLBACK || 'true').toLowerCase() !== 'false';
let sqliteDb;
...
function validPassword(value) {
  ...
  const kinds = [/[a-z]/.test(password), /[A-Z]/.test(password), /\d/.test(password), /[^A-Za-z0-9\s]/.test(password)].filter(Boolean).length;
  return password.length >= 8 && kinds >= 3;
}

function cleanExpiredSecurityState() {
  const now = Date.now();
  for (const [token, session] of sessions) if (session.lastActivity + SESSION_IDLE_TIMEOUT_MS <= now) sessions.delete(token);
...
function publicPayload(payload) {
  ...
  return { ...payload, users: Array.isArray(payload.users) ? payload.users.map(publicUser) : [] };
}

function permissionSet(payload, userId) {
  const user = Array.isArray(payload?.users) ? payload.users.find(item => item.id === userId) : null;
  if (!user) return { user: null, permissions: new Set() };
  if (user.role === '管理员') return { user, permissions: new Set(['*']) };
  const group = Array.isArray(payload?.permissionGroups) ? payload.permissionGroups.find(item => item.id === user.permissionGroupId) : null;
  return { user, permissions: new Set(Array.isArray(group?.permissions) ? group.permissions : []) };
}

function hasAnyPermission(payload, userId, permissions) {
  const current = permissionSet(payload, userId);
  return current.permissions.has('*') || permissions.some(permission => current.permissions.has(permission));
}
...
function requireArrayChangePermission(current, next, key, userId, permissions) {
  ...
  const beforeMap = new Map(before.filter(item => item && item.id != null).map(item => [String(item.id), item]));
  const afterMap = new Map(after.filter(item => item && item.id != null).map(item => [String(item.id), item]));
  const added = [...afterMap.keys()].some(id => !beforeMap.has(id));
  const removed = [...beforeMap.keys()].some(id => !afterMap.has(id));
  const changed = [...afterMap.keys()].some(id => beforeMap.has(id) && !sameValue(beforeMap.get(id), afterMap.get(id)));
...
function preserveAndHashUsers(currentUsers, incomingUsers) {
  const previous = new Map((Array.isArray(currentUsers) ? currentUsers : []).map(user => [user.id, user]));
  ...
  return incomingUsers.map(incoming => {
    ...
    const next = { ...incoming };
    const submittedPassword = typeof incoming?.password === 'string' ? incoming.password : '';
    delete next.password;
    if (current) {
      next.passwordHash = submittedPassword ? passwordHash(submittedPassword) : (current.passwordHash?.startsWith?.('scrypt$') ? current.passwordHash : current.password ? passwordHash(current.password) : '');
    } else {
      if (!validPassword(submittedPassword)) throw new Error('INVALID_NEW_USER_PASSWORD');
...
function normalizeIncomingPayload(currentPayload, incomingPayload, userId) {
  if (!incomingPayload || typeof incomingPayload !== 'object' || Array.isArray(incomingPayload)) throw new Error('INVALID_STATE');
  ...
  if (!actorIsAdmin) {
    ...
    if (!before || !after || before.role !== after.role || before.permissionGroupId !== after.permissionGroupId || before.status !== after.status) throw new Error('FORBIDDEN');
  }
  const changed = key => !sameValue(current[key], next[key]);

  if (changed('users') || changed('permissionGroups')) requireAnyPermission(current, userId, ['users']);
  if (changed('clients')) requireArrayChangePermission(current, next, 'clients', userId, { create: ['clientCreate'], edit: ['clientEdit'], delete: ['clientDelete'] });
  if (changed('clientGroups') || changed('clientSubgroups')) requireAnyPermission(current, userId, ['clientGroups']);
  if (changed('regionCatalog')) requireAnyPermission(current, userId, ['clientProvince', 'clientCity', 'clientDistrict']);
  if (changed('records')) requireArrayChangePermission(current, next, 'records', userId, { create: ['feeCreate'], edit: ['feeEdit'], delete: ['feeDelete'] });
  if (changed('payments')) requireArrayChangePermission(current, next, 'payments', userId, { create: ['paymentCreate', 'feePayment'], edit: ['paymentEdit'], delete: ['paymentDelete'] });
  if (changed('costs')) requireArrayChangePermission(current, next, 'costs', userId, { create: ['costCreate'], edit: ['costEdit'], delete: ['costDelete'] });
  if (changed('dailyExpenses')) requireArrayChangePermission(current, next, 'dailyExpenses', userId, { create: ['dailyExpenseCreate'], edit: ['dailyExpenseEdit'], delete: ['dailyExpenseDelete'] });
  if (changed('customerInfos')) requireArrayChangePermission(current, next, 'customerInfos', userId, { create: ['infoCreate'], edit: ['infoEdit'], delete: ['infoDelete'] });
  if (changed('feeTypes')) requireAnyPermission(current, userId, ['feeTypes']);
  if (changed('employees')) requireAnyPermission(current, userId, ['employees']);
  if (changed('costTypes')) requireAnyPermission(current, userId, ['costTypes']);
  if (changed('suppliers') || changed('supplierDetails')) requireAnyPermission(current, userId, ['costSuppliers']);
  if (changed('reimbursers')) requireAnyPermission(current, userId, ['costReimbursers', 'dailyExpenseReimbursers']);
  if (changed('dailyExpenseTypes')) requireAnyPermission(current, userId, ['dailyExpenseTypes']);
  if (changed('emailSchedule')) requireAnyPermission(current, userId, ['users']);
  return next;
}
...
async function sendReminderEmail(payload, recipients, now, reason = 'scheduled') {
  ...
  await (await getSmtpTransport()).sendMail({ from: smtpConfig.from, to: recipients.join(', '), subject, text: body, headers: { 'X-Xinke-OA-Email': reason } });
...
async function initDatabase() {
  if (storagePreference === 'sqlite') {
    ...
    if (!sqliteModule?.DatabaseSync) throw new Error('SQLite runtime is unavailable');
  ...
  try {
    ...
    sqliteDb = new sqliteModule.DatabaseSync(sqlitePath);
    storageMode = 'sqlite';
    console.warn(`MySQL unavailable (${error.message}); using local SQLite state at ${sqlitePath}.`);
  }
}

async function readState(connection, forUpdate = false) {
  if (storageMode === 'sqlite') {
    const row = sqliteDb.prepare('SELECT version, payload, updated_at FROM app_state WHERE id = 1').get();
  ...
  const [rows] = await activeConnection.query(`SELECT version, payload, updated_at FROM app_state WHERE id = 1${forUpdate ? ' FOR UPDATE' : ''}`);
  const row = rows[0];
  if (!row) return { version: 0, payload: null, updatedAt: null };
  return { version: Number(row.version), payload: JSON.parse(row.payload), updatedAt: row.updated_at };
}
...
async function writeState(expectedVersion, payload) {
  if (storageMode === 'sqlite') {
    ...
    sqliteDb.prepare('UPDATE app_state SET version = ?, payload = ?, updated_at = ? WHERE id = 1').run(nextVersion, JSON.stringify(payload), updatedAt);
...
const allowedUploadExtensions = new Set(['.png', '.jpg', '.jpeg', '.webp', '.gif', '.pdf', '.doc', '.docx', '.xls', '.xlsx', '.txt']);
...
function attachmentPermission(payload, userId, attachmentId) {
  const collections = [
    ...
    ['payments', ['payment', 'paymentCreate', 'paymentEdit', 'paymentDelete', 'feePayment']],
    ['costs', ['cost', 'costCreate', 'costEdit', 'costDelete']],
    ['dailyExpenses', ['dailyExpenses', 'dailyExpenseCreate', 'dailyExpenseEdit', 'dailyExpenseDelete']],
  ];
  for (const [key, permissions] of collections) {
    const item = (payload?.[key] || []).find(record => (record.attachments || []).some(attachment => attachment.id === attachmentId));
...
async function serveUpload(req, res, pathname) {
  ...
  try {
    ...
    res.writeHead(200, { 'content-type': contentTypes[extname(filePath).toLowerCase()] || 'application/octet-stream', 'content-disposition': 'inline', 'cache-control': 'private, max-age=3600', 'x-content-type-options': 'nosniff' });
...
function readJsonBody(req, maxBytes = 1024 * 1024) {
  return new Promise((resolveBody, reject) => {
    ...
    if (Number.isFinite(declaredLength) && declaredLength > maxBytes) { reject(new Error('PAYLOAD_TOO_LARGE')); return; }
    let raw = '';
    let rejected = false;
...
const server = createServer((req, res) => {
  if (req.method === 'OPTIONS') {
    res.writeHead(204, { 'access-control-allow-methods': 'GET, POST, PUT, DELETE, OPTIONS', 'access-control-allow-headers': 'content-type, if-match, authorization' });
    res.end();
    return;
  }
  const pathname = requestPath(req);
  if (pathname === '/api/auth/login' && req.method === 'POST') {
    readJsonBody(req).then(async body => {
      try {
        const state = await readState();
        const identifier = String(body.identifier || '').trim().toLowerCase().replace(/\s+/g, '');
        const password = String(body.password || '');
        const user = (state.payload?.users || []).find(item => [item.username, item.email, item.phone].some(value => String(value || '').trim().toLowerCase().replace(/\s+/g, '') === identifier));
        const credential = user?.passwordHash || user?.password || '';
        if (!user || user.status === '停用' || !verifyPassword(password, credential)) { send(res, 401, { error: 'Invalid credentials' }); return; }
        if (!user.passwordHash) {
          const payload = { ...state.payload, users: state.payload.users.map(item => item.id === user.id ? { ...item, passwordHash: passwordHash(password), password: undefined } : item) };
          const saved = await writeState(state.version, payload);
          if (saved) Object.assign(user, saved.payload.users.find(item => item.id === user.id));
        }
  ...
  if (pathname === '/api/auth/me' && req.method === 'GET') {
    ...
    readState().then(state => {
      ...
      if (!user || user.status === '停用') { sessions.delete(session.token); sendUnauthorized(res); return; }
  ...
  if (pathname === '/api/auth/password' && req.method === 'POST') {
    ...
    readJsonBody(req).then(async body => {
      try {
        const password = String(body.password || '');
        if (!validPassword(password)) { send(res, 400, { error: 'INVALID_PASSWORD' }); return; }
        const state = await readState();
        const { user } = permissionSet(state.payload, session.userId);
        if (!user) { sendUnauthorized(res); return; }
        const payload = { ...state.payload, users: state.payload.users.map(item => item.id === user.id ? { ...item, passwordHash: passwordHash(password), password: undefined, mustChangePassword: false } : item) };
        const saved = await writeState(state.version, payload);
        if (!saved) { send(res, 409, { error: 'State conflict' }); return; }
        send(res, 200, { user: publicUser(saved.payload.users.find(item => item.id === user.id)) });
      } catch (error) { send(res, error?.message === 'FORBIDDEN' ? 403 : 400, { error: 'Password update failed' }); }
  ...
  if (resetPasswordMatch && req.method === 'POST') {
    ...
    readJsonBody(req).then(async body => {
      try {
        ...
        if (!state.payload.users.some(item => item.id === userId)) { send(res, 404, { error: 'User not found' }); return; }
        const payload = { ...state.payload, users: state.payload.users.map(item => item.id === userId ? { ...item, passwordHash: passwordHash(password), password: undefined, mustChangePassword: true } : item) };
        const saved = await writeState(state.version, payload);
        if (!saved) { send(res, 409, { error: 'State conflict' }); return; }
        send(res, 200, { user: publicUser(saved.payload.users.find(item => item.id === userId)) });
      } catch (error) { send(res, error?.message === 'FORBIDDEN' ? 403 : 400, { error: 'Password reset failed' }); }
    }).catch(() => send(res, 400, { error: 'Invalid password payload' }));
    return;
  }
  if (pathname.startsWith('/uploads/')) {
    void serveUpload(req, res, pathname).catch(error => { console.error('Upload read failed:', error?.message || error); send(res, 500, { error: 'File read failed' }); });
    return;
  }
  if (pathname === '/api/uploads' && req.method === 'POST') {
    ...
    readJsonBody(req, maxUploadBodyBytes).then(body => {
      try {
        ...
        if (!name || !match || !isAllowedUpload(name, mime)) { send(res, 400, { error: 'Unsupported file type' }); return; }
        const buffer = Buffer.from(match[1], 'base64');
        if (!buffer.length || buffer.length > maxUploadFileBytes) { send(res, 413, { error: 'File too large' }); return; }
        const extension = extname(name).toLowerCase();
        const filename = `upload-${randomUUID()}${extension}`;
        writeFileSync(resolve(uploadsDir, filename), buffer, { flag: 'wx' });
        pendingUploads.set(filename, { userId: session.userId, expiresAt: Date.now() + SESSION_IDLE_TIMEOUT_MS });
        send(res, 201, { attachment: { id: filename, name, mime: contentTypes[extension] || mime || 'application/octet-stream', size: buffer.length, url: `/uploads/${encodeURIComponent(filename)}` } });
      } catch (error) {
        console.error('Upload failed:', error?.message || error);
        send(res, 400, { error: 'File upload failed' });
      }
    }).catch(error => send(res, error?.message === 'PAYLOAD_TOO_LARGE' ? 413 : 400, { error: error?.message === 'PAYLOAD_TOO_LARGE' ? 'Request too large' : 'Invalid upload' }));
    return;
  }
  if (pathname.startsWith('/api/uploads/') && req.method === 'DELETE') {
    ...
    const filePath = uploadFilePath(filename);
    if (!filePath || basename(filePath) !== filename) { send(res, 400, { error: 'Invalid file path' }); return; }
    readState().then(state => {
      const pending = pendingUploads.get(filename);
      if (!attachmentPermission(state.payload, session.userId, filename) && pending?.userId !== session.userId) { send(res, 403, { error: 'Forbidden' }); return; }
      try { if (existsSync(filePath)) unlinkSync(filePath); pendingUploads.delete(filename); send(res, 204, null); } catch { send(res, 500, { error: 'File delete failed' }); }
    }).catch(() => send(res, 503, { error: 'Database unavailable' }));
    return;
  }
  ...
  if (pathname === '/api/email/test' && req.method === 'POST') {
    ...
    readJsonBody(req).then(async body => {
      try {
        ...
        const status = code === 'SMTP_NOT_CONFIGURED' ? 503 : code === 'INVALID_RECIPIENTS' ? 400 : code === 'FORBIDDEN' ? 403 : 502;
  ...
  readJsonBody(req, STATE_MAX_BYTES).then(body => {
    (async () => {
      try {
        ...
        const current = await readState();
        if (current.version !== expectedVersion) {
          send(res, 409, { error: 'State conflict', version: current.version, payload: publicPayload(current.payload), updatedAt: current.updatedAt });
          return;
        }
        const payload = normalizeIncomingPayload(current.payload, body.payload, session.userId);
        const saved = await writeState(expectedVersion, payload);
        if (!saved) {
          const latest = await readState();
          send(res, 409, { error: 'State conflict', ...latest, payload: publicPayload(latest.payload) });
          return;
        }
        ...
        const invalidUser = ['INVALID_NEW_USER_PASSWORD', 'INVALID_USERS', 'INVALID_STATE'].includes(error?.message);
        send(res, databaseError ? 503 : forbidden ? 403 : invalidUser ? 400 : 400, { error: databaseError ? 'Database unavailable' : forbidden ? 'Forbidden' : invalidUser ? error.message : 'Invalid JSON' });
      }
    })();
  }).catch(error => send(res, error?.message === 'PAYLOAD_TOO_LARGE' ? 413 : 400, { error: error?.message === 'PAYLOAD_TOO_LARGE' ? 'Request too large' : 'Invalid JSON' }));
});
...
async function shutdown(signal) {
  ...
  server.close(() => Promise.all([pool?.end?.(), sqliteDb?.close?.()]).finally(() => process.exit(0)));
}
...
initDatabase().then(() => {
  server.listen(port, '0.0.0.0', () => {
    const storageLabel = storageMode === 'sqlite' ? `SQLite ${sqlitePath}` : `MySQL ${dbConfig.host}:${dbConfig.port}/${dbConfig.database}`;
    console.log(`Xinke state API listening on http://127.0.0.1:${port} (${storageLabel})`);
