...
import {
  ArrowDown, ArrowLeft, ArrowUp, Banknote, CalendarDays, Check, ChevronDown, ChevronRight, CircleDollarSign, Edit3, Mail, Paperclip, TrendingUp,
  LayoutDashboard, LogOut, Menu, Plus, Search, Settings, ShieldCheck, Trash2, UserRound, Users, X, Eye, EyeOff
} from 'lucide-react';
...
type DailyExpense = { id: string; docId: string; recordDate: string; expenseType: string; reimburser: string; amount: number; note: string; attachments?: Attachment[] };
type CustomerInfo = { id: string; clientId: string; name: string; note: string };
type EmailFrequency = 'daily' | 'weekly' | 'monthly';
type EmailSchedule = { enabled: boolean; frequency: EmailFrequency; sendTime: string; weekDay?: number; monthDay?: number; recipients: string[]; lastSentAt?: string; lastAutoSentAt?: string; lastTestAt?: string; lastError?: string };
type EmailServiceStatus = { configured: boolean; from: string | null };
type AuditAction = 'create' | 'update' | 'delete' | 'security' | 'send';
type AuditLog = { id: string; userId: string; username: string; action: AuditAction; entity: string; entityId?: string; summary: string; createdAt: string };
type SupplierDetails = Record<string, { contact: string; phone: string }>;
type PermissionKey = 'dashboard' | 'clients' | 'clientGroups' | 'clientProvince' | 'clientCity' | 'clientDistrict' | 'clientCreate' | 'clientEdit' | 'clientDelete' | 'fee' | 'employees' | 'feeTypes' | 'feeCreate' | 'feeEdit' | 'feePayment' | 'feeDelete' | 'payment' | 'paymentCreate' | 'paymentEdit' | 'paymentDelete' | 'cost' | 'costTypes' | 'costReimbursers' | 'costSuppliers' | 'costCreate' | 'costEdit' | 'costDelete' | 'info' | 'infoCreate' | 'infoEdit' | 'infoDelete' | 'companyExpenses' | 'dailyExpenses' | 'dailyExpenseTypes' | 'dailyExpenseReimbursers' | 'dailyExpenseCreate' | 'dailyExpenseEdit' | 'dailyExpenseDelete' | 'users';
type PermissionGroup = { id: string; name: string; permissions: PermissionKey[] };
type User = { id: string; name: string; username: string; email: string; phone: string; password?: string; mustChangePassword?: boolean; role: Role; status: '正常' | '停用'; permissionGroupId?: string };
...
function normalizeAttachments(value: unknown): Attachment[] {
  ...
  return sourceValue.flatMap((item, index) => {
    ...
    const rawName = String(source.name || source.fileName || source.filename || '').trim();
...
const seedClients: Client[] = [
  { id: 'c1', name: '张伟', company: '星河科技有限公司', phone: '138 0013 8001', email: 'zhangwei@xinghe.com', startDate: '2026-01-01', endDate: '2026-12-31', createdAt: '2026-05-12', province: '内蒙古自治区', city: '鄂尔多斯市', district: '东胜区', customerTypeId: 'g1', groupId: 'g1', subgroupId: 'sg1' },
  { id: 'c2', name: '李娜', company: '远山设计工作室', phone: '139 2345 6789', email: 'lina@yuan.design', startDate: '2026-06-01', endDate: '2026-12-31', createdAt: '2026-06-03', province: '内蒙古自治区', city: '鄂尔多斯市', district: '东胜区', customerTypeId: 'g1', groupId: 'g1', subgroupId: 'sg1' },
  { id: 'c3', name: '王浩', company: '森屿贸易', phone: '186 7654 3210', email: 'wanghao@senyu.cn', startDate: '2026-07-01', endDate: '2026-12-31', createdAt: '2026-07-19', province: '内蒙古自治区', city: '鄂尔多斯市', district: '', customerTypeId: 'g2', groupId: 'g2', subgroupId: 'sg2' },
];
const seedGroups: ClientGroup[] = [{ id: 'g1', name: '重点客户' }, { id: 'g2', name: '常规客户' }];
const seedSubgroups: ClientSubgroup[] = [{ id: 'sg1', groupId: 'g1', name: '默认分类' }, { id: 'sg2', groupId: 'g2', name: '默认分类' }];
const defaultProvince = '内蒙古自治区';
const defaultCity = '鄂尔多斯市';
const defaultDistrict = '';
const defaultRegionCatalog: RegionCatalog = {
  '内蒙古自治区': { '鄂尔多斯市': ['东胜区', '康巴什区', '达拉特旗', '杭锦旗', '乌审旗', '伊金霍洛旗', '鄂托克旗', '鄂托克前旗'] },
...
function migrateClientLocation(client: Client, groups: ClientGroup[], subgroups: ClientSubgroup[]): Client {
  const customerTypeId = client.customerTypeId || client.groupId || groups[0]?.id || '';
  const subgroup = client.subgroupId ? subgroups.find(item => item.id === client.subgroupId) : undefined;
  const legacyDistrict = subgroup && !['直系客户', '李博合作客户', '默认分类'].includes(subgroup.name) ? subgroup.name : undefined;
  return {
    ...client,
    customerTypeId,
    ...
    district: client.district === '未设置区县' ? '' : client.district || legacyDistrict || '',
  };
}
const seedRecords: RecordItem[] = [
  { id: 'r1', docId: '20260101001', clientId: 'c1', start: '2026-01-01', end: '2026-12-31', recordDate: '2026-01-01', fee: 12000, paid: 6000, paymentDate: '2026-01-06', method: '银行转账', feeType: '维护费', note: '年度系统维护服务' },
  { id: 'r2', docId: '20260701001', clientId: 'c1', start: '2026-07-01', end: '2026-09-30', recordDate: '2026-07-01', fee: 2400, paid: 2400, paymentDate: '2026-07-02', method: '微信支付', feeType: '加速器', note: '第三季度增值服务' },
  { id: 'r3', docId: '20260601001', clientId: 'c2', start: '2026-06-01', end: '2026-12-31', recordDate: '2026-06-01', fee: 5600, paid: 2000, paymentDate: '2026-06-08', method: '支付宝', feeType: 'X管家', note: '网站托管及维护' },
];
...
const seedEmailSchedule: EmailSchedule = { enabled: false, frequency: 'daily', sendTime: '09:00', weekDay: 1, monthDay: 1, recipients: [] };
...
const seedUsers: User[] = [
  { id: 'u1', name: '陈思远', username: 'admin', email: 'admin@yunqiao.cn', phone: '138 0000 0001', role: '管理员', status: '正常', permissionGroupId: 'pg1' },
  { id: 'u2', name: '周敏', username: 'finance', email: 'finance@yunqiao.cn', phone: '138 0000 0002', role: '财务', status: '正常', permissionGroupId: 'pg2' },
  { id: 'u3', name: '刘洋', username: 'viewer', email: 'viewer@yunqiao.cn', phone: '138 0000 0003', role: '查看者', status: '正常', permissionGroupId: 'pg3' },
];
const allPermissions: { key: PermissionKey; label: string }[] = [
  { key: 'dashboard', label: '业务总览' }, { key: 'clients', label: '客户列表' },
  { key: 'clientGroups', label: '客户类型分类' }, { key: 'clientProvince', label: '省份/直辖市分类' },
  { key: 'clientCity', label: '地市分类' }, { key: 'clientDistrict', label: '区/县分类' },
  { key: 'clientCreate', label: '添加客户' },
  { key: 'clientEdit', label: '修改客户' }, { key: 'clientDelete', label: '删除客户' },
  { key: 'fee', label: '费用明细' }, { key: 'employees', label: '业务经理管理' }, { key: 'feeTypes', label: '费用明细类型' },
  { key: 'feeCreate', label: '增加费用' }, { key: 'feeEdit', label: '修改记录' },
  { key: 'feePayment', label: '添加回款' }, { key: 'feeDelete', label: '删除记录' },
  { key: 'payment', label: '回款明细' }, { key: 'paymentCreate', label: '添加回款' },
  { key: 'paymentEdit', label: '修改收款记录' }, { key: 'paymentDelete', label: '删除收款记录' },
  { key: 'cost', label: '成本明细' }, { key: 'costTypes', label: '成本费用类型' }, { key: 'costReimbursers', label: '报销人' }, { key: 'costSuppliers', label: '供应商管理' }, { key: 'costCreate', label: '添加成本' },
  { key: 'costEdit', label: '修改成本费用' }, { key: 'costDelete', label: '删除成本费用' },
  { key: 'info', label: '运维资料' }, { key: 'infoCreate', label: '添加运维资料' },
  { key: 'infoEdit', label: '修改资料' }, { key: 'infoDelete', label: '删除资料' },
  { key: 'companyExpenses', label: '公司费用管理' }, { key: 'dailyExpenses', label: '日常费用管理' },
  { key: 'dailyExpenseTypes', label: '费用类型' }, { key: 'dailyExpenseReimbursers', label: '报销人' },
  { key: 'dailyExpenseCreate', label: '添加日常费用' }, { key: 'dailyExpenseEdit', label: '修改日常费用' }, { key: 'dailyExpenseDelete', label: '删除日常费用' },
  { key: 'users', label: '权限管理' },
];
const clientManagementPermissions: PermissionKey[] = ['clientGroups', 'clientProvince', 'clientCity', 'clientDistrict', 'clientCreate', 'clientEdit', 'clientDelete'];
const clientSubPermissions: PermissionKey[] = [...clientManagementPermissions];
const feeSubPermissions: PermissionKey[] = ['employees', 'feeTypes', 'feeCreate', 'feeEdit', 'feePayment', 'feeDelete'];
const feePermissions: PermissionKey[] = ['fee', ...feeSubPermissions];
const paymentSubPermissions: PermissionKey[] = ['paymentCreate', 'paymentEdit', 'paymentDelete'];
const paymentPermissions: PermissionKey[] = ['payment', ...paymentSubPermissions];
const costSubPermissions: PermissionKey[] = ['costTypes', 'costReimbursers', 'costSuppliers', 'costCreate', 'costEdit', 'costDelete'];
const costPermissions: PermissionKey[] = ['cost', ...costSubPermissions];
const infoSubPermissions: PermissionKey[] = ['infoCreate', 'infoEdit', 'infoDelete'];
...
const companyExpensePermissions: PermissionKey[] = ['companyExpenses', ...companyExpenseSubPermissions];
const dailyExpenseSubPermissions: PermissionKey[] = ['dailyExpenseTypes', 'dailyExpenseReimbursers', 'dailyExpenseCreate', 'dailyExpenseEdit', 'dailyExpenseDelete'];
const dailyExpensePermissions: PermissionKey[] = ['dailyExpenses', ...dailyExpenseSubPermissions];
const seedPermissionGroups: PermissionGroup[] = [
  { id: 'pg1', name: '管理员组', permissions: allPermissions.map(item => item.key) },
  { id: 'pg2', name: '财务组', permissions: ['dashboard', 'clients', ...clientManagementPermissions, ...feePermissions, ...paymentPermissions, ...costPermissions, ...infoPermissions, ...companyExpensePermissions, ...dailyExpenseSubPermissions] },
  { id: 'pg3', name: '查看组', permissions: ['dashboard', 'clients', ...clientManagementPermissions, ...feePermissions, ...paymentPermissions, ...costPermissions, ...infoPermissions, ...companyExpensePermissions, ...dailyExpenseSubPermissions] },
];
function migratePermissionGroup(group: PermissionGroup): PermissionGroup {
  ...
  const clientRegionPermissions: PermissionKey[] = ['clientProvince', 'clientCity', 'clientDistrict'];
  if (permissions.has('clientGroups') && !clientRegionPermissions.some(permission => permissions.has(permission))) clientRegionPermissions.forEach(permission => permissions.add(permission));
  [feePermissions, paymentPermissions, costPermissions, infoPermissions, dailyExpensePermissions].forEach(permissionSet => {
    const [parent, ...children] = permissionSet;
    // A saved child permission always implies access to its parent section.
    if (children.some(permission => permissions.has(permission))) permissions.add(parent);
    ...
    if (permissions.has(parent) && !children.some(permission => permissions.has(permission))) children.forEach(permission => permissions.add(permission));
  });
  ...
  if (permissions.has('cost') && !companyExpensePermissions.some(permission => permissions.has(permission))) companyExpensePermissions.forEach(permission => permissions.add(permission));
  if (permissions.has('cost') && !permissions.has('costReimbursers') && !permissions.has('costSuppliers')) {
    permissions.add('costReimbursers');
    permissions.add('costSuppliers');
  }
  if (permissions.has('dailyExpenses') && !dailyExpenseSubPermissions.some(permission => permissions.has(permission))) dailyExpenseSubPermissions.forEach(permission => permissions.add(permission));
  if (permissions.has('dailyExpenses') || dailyExpenseSubPermissions.some(permission => permissions.has(permission))) permissions.add('companyExpenses');
  return permissions.size === group.permissions.length ? group : { ...group, permissions: [...permissions] };
}
...
function readStored<T extends keyof StoredData>(key: T, fallback: StoredData[T]): StoredData[T] {
...
function normalizeSupplierDetails(value: unknown): SupplierDetails {
  ...
  return Object.entries(value as Record<string, unknown>).reduce<SupplierDetails>((result, [name, detail]) => {
    if (detail && typeof detail === 'object' && !Array.isArray(detail)) {
      const item = detail as Record<string, unknown>;
      result[name] = { contact: typeof item.contact === 'string' ? item.contact : '', phone: typeof item.phone === 'string' ? item.phone : '' };
    }
    return result;
  }, {});
}
...
function readNavigationState() {
  const fallback = { active: 'dashboard' as const, selectedId: '', recordTab: 'fee' as const };
  if (typeof window === 'undefined') return fallback;
  try {
    const stored = window.localStorage.getItem(NAVIGATION_STORAGE_KEY);
    const value = stored ? JSON.parse(stored) as Partial<{ active: string; selectedId: string; recordTab: string }> : {};
    return {
      active: value.active === 'clients' || value.active === 'companyExpenses' || value.active === 'dailyExpenses' || value.active === 'users' ? value.active : 'dashboard',
      selectedId: typeof value.selectedId === 'string' ? value.selectedId : '',
      recordTab: value.recordTab === 'payment' || value.recordTab === 'cost' || value.recordTab === 'info' ? value.recordTab : 'fee',
    } as { active: 'dashboard' | 'clients' | 'companyExpenses' | 'dailyExpenses' | 'users'; selectedId: string; recordTab: 'fee' | 'payment' | 'cost' | 'info' };
  } catch {
    return fallback;
  }
}
...
const makeDocId = (date: string, records: RecordItem[]) => {
  ...
  const prefix = normalized.length === 8 ? normalized : new Date().toISOString().slice(0, 10).replace(/-/g, '');
  const next = records.filter(record => record.docId?.startsWith(prefix)).reduce((max, record) => Math.max(max, Number(record.docId.slice(-3)) || 0), 0) + 1;
...
const makeDailyExpenseDocId = (date: string, expenses: DailyExpense[]) => {
  const normalized = date.replace(/-/g, '');
  const prefix = normalized.length === 8 ? normalized : new Date().toISOString().slice(0, 10).replace(/-/g, '');
  const next = expenses.filter(expense => expense.docId?.startsWith(prefix)).reduce((max, expense) => Math.max(max, Number(expense.docId.slice(-4)) || 4999), 4999) + 1;
...
const normalizeLoginIdentifier = (value: string) => value.trim().toLowerCase().replace(/\s+/g, '');
const isPasswordValid = (value: string) => {
  const types = [/[a-z]/.test(value), /[A-Z]/.test(value), /\d/.test(value), /[^A-Za-z0-9\s]/.test(value)].filter(Boolean).length;
...
const isSupportedAttachment = (file: File) => {
  ...
  return file.type.startsWith('image/') || ['.pdf', '.doc', '.docx', '.xls', '.xlsx', '.txt'].includes(extension);
};
...
const formatClientCreatedAt = (value: string) => {
  ...
  const timePart = /T\d{2}:\d{2}/.test(value) ? date.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false }) : '00:00';
  return `${datePart} ${timePart}`;
};
const customerNameLength = (client: Client) => Array.from(client.company || client.name || '').length;
const sortClientsByNameLength = (items: Client[]) => [...items].sort((a, b) => {
  const lengthDifference = customerNameLength(a) - customerNameLength(b);
  return lengthDifference || (a.company || a.name).localeCompare(b.company || b.name, 'zh-CN');
});
const compareTextLength = (left: string, right: string) => {
  const lengthDifference = Array.from(left.trim()).length - Array.from(right.trim()).length;
...
function Modal({ title, onClose, children, className = '' }: { title: string; onClose: () => void; children: React.ReactNode; className?: string }) {
  return <div className="modal-backdrop" onMouseDown={onClose}><div className={`modal ${className || (title === '角色权限管理' ? 'permission-modal' : '')}`} onMouseDown={e => e.stopPropagation()}><div className="modal-head"><h3>{title}</h3><button className="icon-btn" onClick={onClose} aria-label="关闭"><X size={18} /></button></div>{children}</div></div>;
}
...
function RegionManagerModal({ manager, items, regionCatalog, regionLevelLabels, regionName, editingRegionName, canManage, onClose, onParentChange, onNameChange, onEdit, onRemove, onSave }: { manager: { level: RegionLevel; province: string; city: string }; items: string[]; regionCatalog: RegionCatalog; regionLevelLabels: Record<RegionLevel, string>; regionName: string; editingRegionName: string | null; canManage: boolean; onClose: () => void; onParentChange: (field: 'province' | 'city', value: string) => void; onNameChange: (value: string) => void; onEdit: (name: string) => void; onRemove: (name: string) => void; onSave: () => void }) {
  ...
  const cities = sortTextValues(Object.keys(regionCatalog[manager.province] || {}));
  const sortedItems = sortTextValues(items);
  return <Modal title={`${regionLevelLabels[manager.level]}管理`} onClose={onClose} className="region-manager-modal"><div className="region-manager-context"><strong>独立管理{regionLevelLabels[manager.level]}分类</strong><span>新增或修改后会同步到筛选和客户资料。</span></div>{manager.level !== 'province' && <label className="region-manager-parent">所属省份/直辖市<select value={manager.province} onChange={event => onParentChange('province', event.target.value)} disabled={!canManage}>{provinces.map(province => <option key={province} value={province}>{province}</option>)}</select></label>}{manager.level === 'district' && <label className="region-manager-parent">所属地市<select value={manager.city} onChange={event => onParentChange('city', event.target.value)} disabled={!canManage}>{cities.map(city => <option key={city} value={city}>{city}</option>)}</select></label>}<div className="region-manager-list">{sortedItems.map(item => <div className="region-manager-row" key={item}><strong>{item}</strong><div className="region-manager-actions"><button className="icon-btn" onClick={() => onEdit(item)} disabled={!canManage} aria-label={`编辑${item}`} title={`编辑${item}`}><Edit3 size={15} /></button><button className="icon-btn danger" onClick={() => onRemove(item)} disabled={!canManage} aria-label={`删除${item}`} title={`删除${item}`}><Trash2 size={15} /></button></div></div>)}{!items.length && <div className="empty">暂无分类，请先添加</div>}</div><div className="region-manager-add"><input value={regionName} onChange={event => onNameChange(event.target.value)} disabled={!canManage} placeholder={editingRegionName ? `修改${regionLevelLabels[manager.level]}名称` : `新增${regionLevelLabels[manager.level]}名称`} /><button className="secondary-btn" onClick={onSave} disabled={!canManage}>{editingRegionName ? <Check size={15} /> : <Plus size={15} />}{editingRegionName ? '保存修改' : '添加分类'}</button></div><div className="modal-actions"><button className="primary-btn" onClick={onClose}><Check size={16} />完成</button></div></Modal>;
}
...
export default function App() {
  ...
  const [clients, setClients] = useState<Client[]>(() => readStored('clients', seedClients));
  const [regionCatalog, setRegionCatalog] = useState<RegionCatalog>(readRegionCatalog);
  const [clientGroups, setClientGroups] = useState<ClientGroup[]>(() => readStored('clientGroups', seedGroups));
  const [clientSubgroups, setClientSubgroups] = useState<ClientSubgroup[]>(() => readStored('clientSubgroups', seedSubgroups));
  const [records, setRecords] = useState<RecordItem[]>(() => readStored('records', seedRecords).map(record => ({ ...record, attachments: normalizeAttachments(record.attachments) })));
  const [payments, setPayments] = useState<PaymentItem[]>(() => readStored('payments', []).map(payment => ({ ...payment, attachments: normalizeAttachments(payment.attachments) })));
  const [costs, setCosts] = useState<CostItem[]>(() => readStored('costs', []).map(cost => ({ ...cost, costType: cost.costType || cost.feeTypes?.[0], attachments: normalizeAttachments(cost.attachments) })));
  const [dailyExpenses, setDailyExpenses] = useState<DailyExpense[]>(() => readStored('dailyExpenses', []).map(expense => ({ ...expense, attachments: normalizeAttachments(expense.attachments) })));
  const [customerInfos, setCustomerInfos] = useState<CustomerInfo[]>(() => readStored('customerInfos', []));
  const [permissionGroups, setPermissionGroups] = useState<PermissionGroup[]>(() => readStored('permissionGroups', seedPermissionGroups).map(migratePermissionGroup));
  const [feeTypes, setFeeTypes] = useState<string[]>(() => readStored('feeTypes', seedFeeTypes));
  const [employees, setEmployees] = useState<string[]>(() => sortTextValues(readStored('employees', seedEmployees)));
  const [costTypes, setCostTypes] = useState<string[]>(() => readStored('costTypes', seedCostTypes));
  const [dailyExpenseTypes, setDailyExpenseTypes] = useState<string[]>(() => sortTextValues(readStored('dailyExpenseTypes', seedDailyExpenseTypes)));
  const [reimbursers, setReimbursers] = useState<string[]>(() => sortTextValues(readStored('reimbursers', seedReimbursers)));
  const [suppliers, setSuppliers] = useState<string[]>(() => { const stored = readStored('suppliers', seedSuppliers); return sortTextValues(stored.length ? stored : seedSuppliers); });
  const [supplierDetails, setSupplierDetails] = useState<SupplierDetails>(() => normalizeSupplierDetails(readStored('supplierDetails', {})));
  const [emailSchedule, setEmailSchedule] = useState<EmailSchedule>(() => readStored('emailSchedule', seedEmailSchedule));
  const [emailRecipientsInput, setEmailRecipientsInput] = useState(() => readStored('emailSchedule', seedEmailSchedule).recipients.join(', '));
  const [emailServiceStatus, setEmailServiceStatus] = useState<EmailServiceStatus>({ configured: false, from: null });
  ...
  const [directoryCollapsedGroups, setDirectoryCollapsedGroups] = useState<string[]>(() => readStored('clientGroups', seedGroups).map(group => group.id));
  const [directoryCollapsedSubgroups, setDirectoryCollapsedSubgroups] = useState<string[]>(() => readStored('clientSubgroups', seedSubgroups).map(subgroup => subgroup.id));
  ...
  const [regionFilters, setRegionFilters] = useState({ province: '', city: '', district: '', customerTypeId: '' });
  const [clientModal, setClientModal] = useState<Client | 'new' | null>(null);
  const [regionManager, setRegionManager] = useState<{ level: RegionLevel; province: string; city: string } | null>(null);
  const [regionName, setRegionName] = useState('');
  const [editingRegionName, setEditingRegionName] = useState<string | null>(null);
  ...
  const [recordModal, setRecordModal] = useState<RecordItem | 'new' | null>(null);
  const [paymentModal, setPaymentModal] = useState(false);
  const [editingPaymentId, setEditingPaymentId] = useState<string | null>(null);
  const [recordTab, setRecordTab] = useState<'fee' | 'payment' | 'cost' | 'info'>(navigation.recordTab);
  const [customerInfoModal, setCustomerInfoModal] = useState(false);
  const [editingCustomerInfoId, setEditingCustomerInfoId] = useState<string | null>(null);
  ...
  const [permissionRolesCollapsed, setPermissionRolesCollapsed] = useState(false);
  const [permissionAddRoleCollapsed, setPermissionAddRoleCollapsed] = useState(false);
  const [permissionChecksCollapsed, setPermissionChecksCollapsed] = useState(false);
  const [permissionSaveConfirmOpen, setPermissionSaveConfirmOpen] = useState(false);
  ...
  const [profitFilters, setProfitFilters] = useState({ startDate: '', endDate: '', employee: '', clientId: '', docId: '' });
  ...
  const [formRecord, setFormRecord] = useState({ start: '', end: '', recordDate: toLocalDateTimeInputValue().slice(0, 10), fee: '0', paid: '0', paymentDate: toLocalDateTimeInputValue().slice(0, 10), method: '', feeType: '', employee: '', projectName: '', note: '' });
  ...
  const [formPayment, setFormPayment] = useState({ docId: '', paymentDate: toLocalDateTimeInputValue().slice(0, 10), expectedPaymentDate: '', method: '', amount: '', note: '' });
  const [pendingPaymentAttachments, setPendingPaymentAttachments] = useState<PendingAttachment[]>([]);
  const [savedPaymentAttachments, setSavedPaymentAttachments] = useState<Attachment[]>([]);
  const [paymentAttachmentsToDelete, setPaymentAttachmentsToDelete] = useState<Attachment[]>([]);
  const [paymentSaving, setPaymentSaving] = useState(false);
  const [costModal, setCostModal] = useState<CostItem | 'new' | null>(null);
  const [formCost, setFormCost] = useState({ docId: '', supplier: '', reimburser: '', costType: '', amount: '', note: '', createdAt: '' });
  const [pendingCostAttachments, setPendingCostAttachments] = useState<PendingAttachment[]>([]);
  const [savedCostAttachments, setSavedCostAttachments] = useState<Attachment[]>([]);
  const [costAttachmentsToDelete, setCostAttachmentsToDelete] = useState<Attachment[]>([]);
  const [costSaving, setCostSaving] = useState(false);
  const [dailyExpenseModal, setDailyExpenseModal] = useState<DailyExpense | 'new' | null>(null);
  const [formDailyExpense, setFormDailyExpense] = useState({ recordDate: toLocalDateTimeInputValue().slice(0, 10), expenseType: '', reimburser: '', amount: '', note: '' });
  const [pendingDailyExpenseAttachments, setPendingDailyExpenseAttachments] = useState<PendingAttachment[]>([]);
  const [savedDailyExpenseAttachments, setSavedDailyExpenseAttachments] = useState<Attachment[]>([]);
  const [dailyExpenseAttachmentsToDelete, setDailyExpenseAttachmentsToDelete] = useState<Attachment[]>([]);
  const [dailyExpenseSaving, setDailyExpenseSaving] = useState(false);
  const [dailyExpenseTypeName, setDailyExpenseTypeName] = useState('');
  const [editingDailyExpenseType, setEditingDailyExpenseType] = useState<string | null>(null);
  const [dailyExpenseTypeManagerOpen, setDailyExpenseTypeManagerOpen] = useState(false);
  const [reimburserName, setReimburserName] = useState('');
  const [editingReimburser, setEditingReimburser] = useState<string | null>(null);
  const [reimburserManagerOpen, setReimburserManagerOpen] = useState(false);
  const [reimburserManagerScope, setReimburserManagerScope] = useState<'daily' | 'cost'>('daily');
  const [companyExpenseFilters, setCompanyExpenseFilters] = useState({ startDate: '', endDate: '', expenseType: '', reimburser: '' });
  ...
  const [formCustomerInfo, setFormCustomerInfo] = useState({ name: '', note: '' });
  const [formUser, setFormUser] = useState({ name: '', username: '', email: '', phone: '', password: '', role: '财务' as Role });
  const [permissionGroupName, setPermissionGroupName] = useState('');
  const [editingPermissionGroupId, setEditingPermissionGroupId] = useState<string | null>(null);
  const [permissionGroupPermissions, setPermissionGroupPermissions] = useState<PermissionKey[]>(allPermissions.map(item => item.key));
  const [toast, setToast] = useState('');
  const sessionPermissionGroup = session?.permissionGroupId ? permissionGroups.find(group => group.id === session.permissionGroupId) : undefined;
  const canAccess = (permission: PermissionKey) => session?.role === '管理员' || !sessionPermissionGroup || sessionPermissionGroup.permissions.includes(permission);
  const canManageClientGroups = session?.role !== '查看者' && canAccess('clientGroups');
  const canManageClientProvince = session?.role !== '查看者' && canAccess('clientProvince');
  const canManageClientCity = session?.role !== '查看者' && canAccess('clientCity');
  const canManageClientDistrict = session?.role !== '查看者' && canAccess('clientDistrict');
  const canManageRegionLevel = (level: RegionLevel) => level === 'province' ? canManageClientProvince : level === 'city' ? canManageClientCity : canManageClientDistrict;
  const canCreateClient = session?.role !== '查看者' && canAccess('clientCreate');
  const canEditClient = session?.role !== '查看者' && canAccess('clientEdit');
  const canDeleteClient = session?.role !== '查看者' && canAccess('clientDelete');
  ...
  const canManageDailyExpenseTypes = session?.role !== '查看者' && canAccess('dailyExpenseTypes');
  const canManageDailyExpenseReimbursers = session?.role !== '查看者' && canAccess('dailyExpenseReimbursers');
  const canCreateDailyExpense = session?.role !== '查看者' && canAccess('dailyExpenseCreate');
  const canEditDailyExpense = session?.role !== '查看者' && canAccess('dailyExpenseEdit');
  const canDeleteDailyExpense = session?.role !== '查看者' && canAccess('dailyExpenseDelete');
  ...
  const canManageEmployees = session?.role !== '查看者' && canAccess('employees');
  const canManageCostTypes = session?.role !== '查看者' && canAccess('costTypes');
  const canManageCostReimbursers = session?.role !== '查看者' && canAccess('costReimbursers');
  const canManageCostSuppliers = session?.role !== '查看者' && canAccess('costSuppliers');
  const canManageActiveReimbursers = reimburserManagerScope === 'cost' ? canManageCostReimbursers : canManageDailyExpenseReimbursers;
  const canCreateFee = session?.role !== '查看者' && canAccess('feeCreate');
  const canEditFee = session?.role !== '查看者' && canAccess('feeEdit');
  const canEditFeePayment = session?.role !== '查看者' && canAccess('feePayment');
  const canDeleteFee = session?.role !== '查看者' && canAccess('feeDelete');
  const canCreatePayment = session?.role !== '查看者' && canAccess('paymentCreate');
  const canEditPaymentRecord = session?.role !== '查看者' && canAccess('paymentEdit');
  const canDeletePayment = session?.role !== '查看者' && canAccess('paymentDelete');
  ...
  const preferredRecordTab: 'fee' | 'payment' | 'cost' | 'info' = canViewFee ? 'fee' : canViewPayment ? 'payment' : canViewCost ? 'cost' : 'info';
  const visibleRecordTab = recordTab === 'fee' ? (canViewFee ? recordTab : preferredRecordTab) : recordTab === 'payment' ? (canViewPayment ? recordTab : preferredRecordTab) : recordTab === 'cost' ? (canViewCost ? recordTab : preferredRecordTab) : (canViewInfo ? recordTab : preferredRecordTab);
  ...
  const selectedCostRecords = costRecordSelection ? selectedRecords.filter(record => record.docId === costRecordSelection) : selectedRecords;
  const selectedCostItems = selectedCosts.filter(cost => selectedCostRecords.some(record => record.docId === cost.docId));
  ...
  const totals = useMemo(() => ({ total: records.reduce((s, r) => s + r.fee, 0), paid: records.reduce((s, r) => s + paidFor(r), 0) }), [records, payments]);
  const reminderRecords = useMemo(() => records.filter(record => { const days = daysUntil(record.paymentDate); return record.fee > paidFor(record) && days !== null && days <= 10; }), [records, payments]);
  const latestClients = useMemo(() => [...clients].sort((a, b) => {
    const dateDifference = new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
    return (Number.isNaN(dateDifference) ? 0 : dateDifference) || a.company.localeCompare(b.company, 'zh-CN');
  }).slice(0, 100), [clients]);
  const recentClientsPageCount = Math.max(1, Math.ceil(latestClients.length / recentClientsPageSize));
  const recentClientsPageEntries = latestClients.slice((recentClientsPage - 1) * recentClientsPageSize, recentClientsPage * recentClientsPageSize);
  const remindersPageCount = Math.max(1, Math.ceil(reminderRecords.length / remindersPageSize));
  const remindersPageEntries = reminderRecords.slice((remindersPage - 1) * remindersPageSize, remindersPage * remindersPageSize);
  const profitAnalysisEmployees = useMemo(() => sortTextValues([...new Set(records.map(record => record.employee).filter(Boolean) as string[])]), [records]);
  const profitAnalysisClients = useMemo(() => [...clients].sort((left, right) => left.company.localeCompare(right.company, 'zh-CN')), [clients]);
  const profitAnalysisDocumentOptions = useMemo(() => records.filter(record => {
    const date = record.recordDate || record.start;
    return (!profitFilters.startDate || date >= profitFilters.startDate)
      && (!profitFilters.endDate || date <= profitFilters.endDate)
      && (!profitFilters.employee || record.employee === profitFilters.employee)
      && (!profitFilters.clientId || record.clientId === profitFilters.clientId);
  }).sort((left, right) => (right.recordDate || right.start).localeCompare(left.recordDate || left.start)), [records, profitFilters.startDate, profitFilters.endDate, profitFilters.employee, profitFilters.clientId]);
  const profitAnalysisRows = useMemo(() => records.filter(record => {
    const date = record.recordDate || record.start;
    return (!profitFilters.startDate || date >= profitFilters.startDate)
      && (!profitFilters.endDate || date <= profitFilters.endDate)
      && (!profitFilters.employee || record.employee === profitFilters.employee)
    ...
    const cost = costs.filter(item => item.clientId === record.clientId && item.docId === record.docId).reduce((sum, item) => sum + item.amount, 0);
    const received = record.paid + payments.filter(payment => payment.clientId === record.clientId && payment.docId === record.docId).reduce((sum, payment) => sum + payment.amount, 0);
    return { id: record.id, docId: record.docId, date: record.recordDate || record.start, client: clients.find(client => client.id === record.clientId)?.company || '未知客户', employee: record.employee || '未设置', revenue, cost, profit: revenue - cost, arrears: Math.max(0, revenue - received) };
  }).sort((left, right) => right.date.localeCompare(left.date)), [records, costs, payments, clients, profitFilters]);
  const profitAnalysisSummary = useMemo(() => profitAnalysisRows.reduce((summary, row) => ({ revenue: summary.revenue + row.revenue, cost: summary.cost + row.cost, profit: summary.profit + row.profit, arrears: summary.arrears + row.arrears }), { revenue: 0, cost: 0, profit: 0, arrears: 0 }), [profitAnalysisRows]);
  const profitAnalysisPageCount = Math.max(1, Math.ceil(profitAnalysisRows.length / profitAnalysisPageSize));
  const profitAnalysisPageEntries = profitAnalysisRows.slice((profitAnalysisPage - 1) * profitAnalysisPageSize, profitAnalysisPage * profitAnalysisPageSize);
  useEffect(() => { setProfitAnalysisPage(1); }, [profitFilters]);
  useEffect(() => { setProfitAnalysisPage(current => Math.min(current, profitAnalysisPageCount)); }, [profitAnalysisPageCount]);
  const companyExpenseRows = useMemo(() => dailyExpenses.filter(expense => (
    (!companyExpenseFilters.startDate || expense.recordDate >= companyExpenseFilters.startDate)
    && (!companyExpenseFilters.endDate || expense.recordDate <= companyExpenseFilters.endDate)
    && (!companyExpenseFilters.expenseType || expense.expenseType === companyExpenseFilters.expenseType)
    && (!companyExpenseFilters.reimburser || expense.reimburser === companyExpenseFilters.reimburser)
  )).sort((left, right) => right.recordDate.localeCompare(left.recordDate) || right.docId.localeCompare(left.docId)), [dailyExpenses, companyExpenseFilters]);
  const companyExpenseTotal = useMemo(() => companyExpenseRows.reduce((sum, expense) => sum + Number(expense.amount || 0), 0), [companyExpenseRows]);
  const companyExpenseAverage = companyExpenseRows.length ? companyExpenseTotal / companyExpenseRows.length : 0;
  const companyExpensePageCount = Math.max(1, Math.ceil(companyExpenseRows.length / companyExpensePageSize));
  const companyExpensePageEntries = companyExpenseRows.slice((companyExpensePage - 1) * companyExpensePageSize, companyExpensePage * companyExpensePageSize);
  useEffect(() => { setCompanyExpensePage(1); }, [companyExpenseFilters]);
  useEffect(() => { setCompanyExpensePage(current => Math.min(current, companyExpensePageCount)); }, [companyExpensePageCount]);
  const dailyExpenseRows = useMemo(() => [...dailyExpenses].sort((left, right) => right.recordDate.localeCompare(left.recordDate) || right.docId.localeCompare(left.docId)), [dailyExpenses]);
  const dailyExpensePageCount = Math.max(1, Math.ceil(dailyExpenseRows.length / dailyExpensePageSize));
  const dailyExpensePageEntries = dailyExpenseRows.slice((dailyExpensePage - 1) * dailyExpensePageSize, dailyExpensePage * dailyExpensePageSize);
  useEffect(() => { setDailyExpensePage(current => Math.min(current, dailyExpensePageCount)); }, [dailyExpensePageCount]);
  const searchActive = search.trim().length > 0;
  const searchFilteredClients = clients.filter(c => {
    const typeName = clientGroups.find(group => group.id === (c.customerTypeId || c.groupId))?.name || '';
    return [c.name, c.company, c.phone, c.email, c.province || '', c.city || '', c.district || '', typeName].some(v => v.toLowerCase().includes(search.toLowerCase()));
  });
  const cityFilterClients = searchFilteredClients.filter(client => !regionFilters.province || client.province === regionFilters.province);
  const districtFilterClients = cityFilterClients.filter(client => !regionFilters.city || client.city === regionFilters.city);
  const typeFilterClients = districtFilterClients.filter(client => !regionFilters.district || client.district === regionFilters.district);
  const regionProvinceOptions = sortTextValues([...new Set([...Object.keys(regionCatalog), ...searchFilteredClients.map(client => client.province).filter(Boolean) as string[]])]);
  const regionCityOptions = sortTextValues([...new Set([...Object.keys(regionCatalog[regionFilters.province] || {}), ...cityFilterClients.map(client => client.city).filter(Boolean) as string[]])]);
  const regionDistrictOptions = sortTextValues([...new Set([...(regionCatalog[regionFilters.province]?.[regionFilters.city] || []), ...districtFilterClients.map(client => client.district).filter(Boolean) as string[]])]);
  const regionTypeOptions = clientGroups.filter(type => typeFilterClients.some(client => (client.customerTypeId || client.groupId) === type.id)).sort((a, b) => compareTextLength(a.name, b.name));
  const managedRegionItems = getManagedRegionItems(regionManager);
  const filteredClients = typeFilterClients.filter(client => !regionFilters.customerTypeId || (client.customerTypeId || client.groupId) === regionFilters.customerTypeId);
  ...
  const sortedClientRows = [...filteredClients].sort((a, b) => {
    const typeName = (client: Client) => clientGroups.find(group => group.id === (client.customerTypeId || client.groupId))?.name || '未设置客户类型';
    const left = [a.province || '未设置省份', a.city || '未设置地市', a.district || '未设置区县', typeName(a), a.company || a.name];
    const right = [b.province || '未设置省份', b.city || '未设置地市', b.district || '未设置区县', typeName(b), b.company || b.name];
  ...
  const clientPageEntries = sortedClientRows.slice((clientPage - 1) * clientPageSize, clientPage * clientPageSize);
  const notify = (message: string) => { setToast(message); window.setTimeout(() => setToast(''), 2200); };
  const recordAudit = (action: AuditAction, entity: string, summary: string, entityId?: string) => {
    if (!session) return;
    setAuditLogs(current => [{ id: `log-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, userId: session.id, username: session.username || session.name, action, entity, entityId, summary, createdAt: new Date().toISOString() }, ...current].slice(0, 1000));
  };
  const getDataSnapshot = (): StoredData => ({ clients, clientGroups, clientSubgroups, records, payments, costs, dailyExpenses, customerInfos, permissionGroups, feeTypes, employees, costTypes, dailyExpenseTypes, reimbursers, suppliers, supplierDetails, emailSchedule, auditLogs, users, regionCatalog });
  const applyServerData = (data: Partial<StoredData>) => {
    if (data.clients) setClients(data.clients.map(client => migrateClientLocation(client, data.clientGroups || clientGroups, data.clientSubgroups || clientSubgroups)));
    if (data.clientGroups) setClientGroups(data.clientGroups);
    if (data.clientSubgroups) setClientSubgroups(data.clientSubgroups);
    if (data.records) setRecords(data.records.map(record => ({ ...record, attachments: normalizeAttachments(record.attachments) })));
    if (data.payments) setPayments(data.payments.map(payment => ({ ...payment, attachments: normalizeAttachments(payment.attachments) })));
    if (data.costs) setCosts(data.costs.map(cost => ({ ...cost, attachments: normalizeAttachments(cost.attachments) })));
    if (data.dailyExpenses) setDailyExpenses(data.dailyExpenses.map(expense => ({ ...expense, attachments: normalizeAttachments(expense.attachments) })));
    if (data.customerInfos) setCustomerInfos(data.customerInfos);
    if (data.permissionGroups) setPermissionGroups(data.permissionGroups.map(migratePermissionGroup));
    ...
    if (data.dailyExpenseTypes) setDailyExpenseTypes(sortTextValues(data.dailyExpenseTypes.length ? data.dailyExpenseTypes : seedDailyExpenseTypes));
    if (data.reimbursers) setReimbursers(sortTextValues(data.reimbursers.length ? data.reimbursers : seedReimbursers));
    if (data.suppliers) setSuppliers(sortTextValues(data.suppliers.length ? data.suppliers : seedSuppliers));
    if (data.supplierDetails) setSupplierDetails(normalizeSupplierDetails(data.supplierDetails));
    if (data.emailSchedule) {
      setEmailSchedule({ ...data.emailSchedule, weekDay: data.emailSchedule.weekDay ?? 1, monthDay: data.emailSchedule.monthDay ?? 1 });
  ...
  const parseRecipients = (value: string) => [...new Set(value.split(/[;,\s]+/).map(item => item.trim().toLowerCase()).filter(Boolean))];
  const openReminderDraft = async () => {
    const recipients = parseRecipients(emailRecipientsInput);
    if (!recipients.length) { notify('请先填写至少一个邮箱地址'); return; }
    if (recipients.some(email => !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))) { notify('请输入正确的邮箱地址'); return; }
    try {
      const response = await apiFetch('/api/email/test', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ recipients }) });
      const result = await response.json() as { error?: string; sent?: boolean };
      if (!response.ok || !result.sent) { notify(result.error || '邮件发送失败'); return; }
      setEmailSchedule(current => ({ ...current, recipients, lastTestAt: new Date().toISOString() }));
      recordAudit('send', '收款提醒邮件', `测试发送收款提醒至：${recipients.join(', ')}`);
      notify('测试邮件已发送');
    } catch {
  ...
  const saveEmailSchedule = () => {
    ...
    recordAudit('update', '定时邮件设置', `修改定时邮件设置：${emailSchedule.frequency}，${recipients.length} 个收件邮箱`);
    notify('定时邮件设置已保存');
  };
  const setChildPermission = (current: PermissionKey[], parent: PermissionKey, children: PermissionKey[], permission: PermissionKey, enabled: boolean) => {
    if (enabled) return [...new Set<PermissionKey>([...current, parent, permission])];
    const next = current.filter(key => key !== permission);
    return children.some(key => next.includes(key)) ? next : next.filter(key => key !== parent);
  };
  const setPermissionEnabled = (permission: PermissionKey, enabled: boolean) => setPermissionGroupPermissions(current => {
    if (permission === 'clients') return enabled ? [...new Set<PermissionKey>([...current, 'clients', ...clientSubPermissions])] : current.filter(key => key !== 'clients' && !clientSubPermissions.includes(key));
    if (permission === 'fee') return enabled ? [...new Set<PermissionKey>([...current, 'fee', ...feeSubPermissions])] : current.filter(key => key !== 'fee' && !feeSubPermissions.includes(key));
    if (permission === 'payment') return enabled ? [...new Set<PermissionKey>([...current, 'payment', ...paymentSubPermissions])] : current.filter(key => key !== 'payment' && !paymentSubPermissions.includes(key));
    if (permission === 'cost') return enabled ? [...new Set<PermissionKey>([...current, 'cost', ...costSubPermissions])] : current.filter(key => key !== 'cost' && !costSubPermissions.includes(key));
    if (permission === 'info') return enabled ? [...new Set<PermissionKey>([...current, 'info', ...infoSubPermissions])] : current.filter(key => key !== 'info' && !infoSubPermissions.includes(key));
    if (permission === 'companyExpenses') return enabled ? [...new Set<PermissionKey>([...current, 'companyExpenses', ...dailyExpensePermissions])] : current.filter(key => key !== 'companyExpenses' && !dailyExpensePermissions.includes(key));
    if (permission === 'dailyExpenses') return enabled ? [...new Set<PermissionKey>([...current, 'companyExpenses', ...dailyExpensePermissions])] : current.filter(key => key !== 'dailyExpenses' && !dailyExpenseSubPermissions.includes(key));
    if (clientSubPermissions.includes(permission)) return setChildPermission(current, 'clients', clientSubPermissions, permission, enabled);
    if (feeSubPermissions.includes(permission)) return setChildPermission(current, 'fee', feeSubPermissions, permission, enabled);
    if (paymentSubPermissions.includes(permission)) return setChildPermission(current, 'payment', paymentSubPermissions, permission, enabled);
    if (costSubPermissions.includes(permission)) return setChildPermission(current, 'cost', costSubPermissions, permission, enabled);
    if (infoSubPermissions.includes(permission)) return setChildPermission(current, 'info', infoSubPermissions, permission, enabled);
    if (dailyExpenseSubPermissions.includes(permission)) {
      const next = setChildPermission(current, 'dailyExpenses', dailyExpenseSubPermissions, permission, enabled);
      return next.includes('dailyExpenses') ? [...new Set<PermissionKey>([...next, 'companyExpenses'])] : next;
    }
    return enabled ? [...new Set<PermissionKey>([...current, permission])] : current.filter(key => key !== permission);
  });
  ...
  useEffect(() => {
    if (!session) { setServerHydrated(false); return; }
    let cancelled = false;
    apiFetch('/api/state', { headers: { accept: 'application/json' } }).then(async response => {
      if (!response.ok) throw new Error('state api unavailable');
      return response.json() as Promise<{ version: number; payload: StoredData | null }>;
    }).then(remote => {
      if (cancelled) return;
  ...
  useEffect(() => {
    apiFetch('/api/email/status', { headers: { accept: 'application/json' } }).then(response => response.ok ? response.json() : null).then(status => {
      if (status?.configured !== undefined) setEmailServiceStatus(status as EmailServiceStatus);
    }).catch(() => {});
  }, []);
    ...
    serverSyncTimer.current = window.setTimeout(async () => {
      ...
      try {
        const response = await apiFetch('/api/state', {
          method: 'PUT',
          headers: { 'content-type': 'application/json', 'if-match': String(serverVersion.current ?? 0) },
          body: JSON.stringify({ payload: getDataSnapshot() })
        });
        const result = await response.json() as { version?: number; payload?: StoredData; error?: string };
        if (response.status === 409 && result.payload) {
          ...
          const mergeById = <T extends { id: string }>(remoteItems: T[] | undefined, localItems: T[]) => {
            const localById = new Map(localItems.map(item => [item.id, item]));
            const merged = (remoteItems || []).map(item => localById.get(item.id) || item);
  ...
  useEffect(() => {
    ...
    const poll = window.setInterval(async () => {
      ...
      try {
        ...
        const remote = await response.json() as { version: number; payload: StoredData | null };
        if (remote.payload && serverVersion.current !== null && remote.version !== serverVersion.current) {
          serverVersion.current = remote.version;
          skipServerSync.current = true;
          applyServerData(remote.payload);
  ...
  useEffect(() => {
    try {
      const data: StoredData = { clients, clientGroups, clientSubgroups, records, payments, costs, dailyExpenses, customerInfos, permissionGroups, feeTypes, employees, costTypes, dailyExpenseTypes, reimbursers, suppliers, supplierDetails, emailSchedule, auditLogs, users, regionCatalog };
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
    } catch {
      // Storage can be unavailable in private browsing or when quota is exceeded.
    }
  }, [clients, clientGroups, clientSubgroups, records, payments, costs, dailyExpenses, customerInfos, permissionGroups, feeTypes, employees, costTypes, dailyExpenseTypes, reimbursers, suppliers, supplierDetails, emailSchedule, auditLogs, users, regionCatalog]);

  useEffect(() => {
    try {
      window.localStorage.setItem(REGION_CATALOG_STORAGE_KEY, JSON.stringify(regionCatalog));
    } catch {
  ...
  useEffect(() => {
    setClients(current => {
      const migrated = current.map(client => migrateClientLocation(client, clientGroups, clientSubgroups));
      return migrated.some((client, index) => JSON.stringify(client) !== JSON.stringify(current[index])) ? migrated : current;
    });
  }, [clientGroups, clientSubgroups]);

  useEffect(() => {
    setDirectoryCollapsedGroups(current => [...new Set([...current, ...clientGroups.map(group => group.id)])]);
  }, [clientGroups]);

  useEffect(() => {
    setDirectoryCollapsedSubgroups(current => [...new Set([...current, ...clientSubgroups.map(subgroup => subgroup.id)])]);
  }, [clientSubgroups]);
  ...
  useEffect(() => {
    if (!session) return;
    let lastRecordedActivity = Number(window.localStorage.getItem(SESSION_ACTIVITY_STORAGE_KEY)) || Date.now();
    ...
    const scheduleLogout = () => {
      ...
      else idleTimer = window.setTimeout(logoutForIdle, SESSION_IDLE_TIMEOUT_MS - elapsed);
    };
    ...
    const syncActivity = () => {
      const storedActivity = Number(window.localStorage.getItem(SESSION_ACTIVITY_STORAGE_KEY));
      if (Number.isFinite(storedActivity) && storedActivity > lastRecordedActivity) {
  ...
  useEffect(() => {
    setAuditPage(current => Math.min(current, Math.max(1, Math.ceil(auditLogs.length / auditPageSize))));
  }, [auditLogs.length, auditPageSize]);

  useEffect(() => {
    setRecentClientsPage(current => Math.min(current, recentClientsPageCount));
  ...
  useEffect(() => {
    if (!session) return;
    const canStay = active === 'dashboard' ? canViewDashboard : active === 'clients' ? canViewClients : active === 'companyExpenses' ? canViewCompanyExpenses : active === 'dailyExpenses' ? canViewDailyExpenses : canViewUsers;
    if (!canStay) setActive(canViewDashboard ? 'dashboard' : canViewClients ? 'clients' : canViewCompanyExpenses ? 'companyExpenses' : canViewDailyExpenses ? 'dailyExpenses' : 'users');
  }, [session?.id, session?.permissionGroupId, permissionGroups, active, canViewDashboard, canViewClients, canViewCompanyExpenses, canViewDailyExpenses, canViewUsers]);
  ...
  useEffect(() => {
    ...
    const record = selectedRecords.find(item => item.docId === formPayment.docId);
    if (record && !formPayment.expectedPaymentDate) {
      setFormPayment(current => ({ ...current, expectedPaymentDate: record.paymentDate || '' }));
    }
  }, [paymentModal, formPayment.docId, selectedRecords, formPayment.expectedPaymentDate]);

  useEffect(() => {
    try {
      window.localStorage.setItem(NAVIGATION_STORAGE_KEY, JSON.stringify({ active, selectedId, recordTab }));
    } catch {
  ...
  useEffect(() => {
    if (active !== 'clients' || clientsExpanded !== previousClientsExpanded.current) setSelectedId('');
    if (!clientsExpanded) {
      setExpandedGroups([]);
  ...
  useEffect(() => {
    ...
    const details = editingSupplier ? supplierDetails[editingSupplier] : undefined;
  ...
  useEffect(() => {
    ...
    if (active === 'clients' && title) title.textContent = selected ? '客户详情' : '客户列表';
    if (active === 'users' && title) title.textContent = '权限管理';
    if (active === 'companyExpenses' && title) title.textContent = '公司费用管理';
    if (active === 'dailyExpenses' && title) title.textContent = '日常费用管理';
  }, [active, selected]);

  useEffect(() => {
    const groups = Array.from(document.querySelectorAll<HTMLElement>('.app-sidebar .nav-group'));
    const companyGroup = groups.find(group => group.querySelector(':scope > button')?.textContent?.includes('公司费用管理'));
    const parent = companyGroup?.querySelector<HTMLButtonElement>(':scope > button');
    const subNav = companyGroup?.querySelector<HTMLElement>(':scope > .sub-nav');
    ...
    subNav.style.display = companyExpensesExpanded && canViewDailyExpenses ? '' : 'none';
  ...
  useEffect(() => {
    const permissionClasses = ['no-permission-dashboard', 'no-permission-clients', 'no-permission-users', 'no-permission-fee', 'no-permission-payment', 'no-permission-cost', 'no-permission-info'];
    permissionClasses.forEach(className => document.body.classList.remove(className));
    if (session) {
      if (!canViewDashboard) document.body.classList.add('no-permission-dashboard');
    ...
    return () => permissionClasses.forEach(className => document.body.classList.remove(className));
  }, [session?.id, canViewDashboard, canViewClients, canViewUsers, canViewFee, canViewPayment, canViewCost, canViewInfo]);

  const auditSearchEntries = useMemo(() => {
    const query = auditSearch.trim().toLocaleLowerCase();
    if (!query) return auditLogs;
    const actionLabels: Record<AuditAction, string> = { create: '新增', update: '修改', delete: '删除', security: '安全操作', send: '发送' };
  ...
  const auditPageCount = Math.max(1, Math.ceil(auditSearchEntries.length / auditPageSize));
  const auditPageEntries = auditSearchEntries.slice((auditPage - 1) * auditPageSize, auditPage * auditPageSize);
  const auditPageNumbers = Array.from({ length: auditPageCount }, (_, index) => index + 1).filter(page => auditPageCount <= 7 || page === 1 || page === auditPageCount || Math.abs(page - auditPage) <= 1);
  useEffect(() => {
 ...
 if (!session) return <div className="login-page"><div className="login-panel"><div className="login-brand"><div className="brand-mark"><CircleDollarSign size={22} /></div><div><strong>OA帮</strong><span>费用客户管理系统</span></div></div><div className="login-copy"><p className="eyebrow">SECURE WORKSPACE</p><h1>让每一笔维护费<br /><em>清晰可追溯</em></h1><p>集中管理客户、服务周期与收款记录，实时掌握应收余额。</p></div><form onSubmit={async e => { e.preventDefault(); try { const response = await apiFetch('/api/auth/login', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ identifier: login.identifier, password: login.password }) }); const result = await response.json() as { user?: User; error?: string }; if (!response.ok || !result.user) { setLoginError('登录信息或密码不正确'); return; } setLoginError(''); setLogin({ identifier: '', password: '' }); setActive('dashboard'); setSelectedId(''); setClientsExpanded(false); setCompanyExpensesExpanded(false); setSession(result.user); } catch { setLoginError('登录服务暂时不可用'); } }}><label>用户名<input type="text" value={login.identifier} placeholder="用户名 / 邮箱 / 手机号" autoComplete="username" onChange={e => { setLogin({ ...login, identifier: e.target.value }); setLoginError(''); }} /></label><label>登录密码<div className="login-password-field"><input type={loginPasswordVisible ? 'text' : 'password'} value={login.password} autoComplete="current-password" onChange={e => { setLogin({ ...login, password: e.target.value }); setLoginError(''); }} /><button type="button" className="login-password-toggle" onClick={() => setLoginPasswordVisible(current => !current)} aria-label={loginPasswordVisible ? '隐藏密码' : '显示密码'} title={loginPasswordVisible ? '隐藏密码' : '显示密码'}>{loginPasswordVisible ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label>{loginError && <div className="login-error" role="alert">{loginError}</div>}<button className="primary-btn login-btn"><ShieldCheck size={17} />进入工作台</button></form><div className="login-foot">支持使用用户名、邮箱或手机号登录</div></div><div className="login-aside" aria-hidden="true" /></div>;

  function openClient(c: Client | 'new') {
    if (c === 'new' ? !canCreateClient : !canEditClient) { notify('当前角色没有此操作权限'); return; }
    const customerTypeId = c === 'new' ? clientGroups[0]?.id ?? '' : c.customerTypeId || c.groupId;
    setClientModal(c);
    setFormClient(c === 'new'
      ? { name: '', company: '', phone: '', email: '', startDate: '', endDate: '', province: defaultProvince, city: defaultCity, district: defaultDistrict, customerTypeId, groupId: customerTypeId, subgroupId: '' }
      : { name: c.name, company: c.company, phone: c.phone, email: c.email, startDate: c.startDate, endDate: c.endDate, province: c.province || '', city: c.city || '', district: c.district || '', customerTypeId, groupId: customerTypeId, subgroupId: c.subgroupId || '' });
  }
  function saveClient() {
    if (!formClient.name.trim() || !formClient.company.trim()) { notify('请填写公司名称和客户姓名'); return; }
    if (!formClient.customerTypeId) { notify('请选择客户类型'); return; }
    if (formClient.startDate && formClient.endDate && formClient.endDate < formClient.startDate) { notify('结束时间不能早于开始时间'); return; }
    const nextClient = { ...formClient, groupId: formClient.customerTypeId, customerTypeId: formClient.customerTypeId };
    if (clientModal === 'new') {
  ...
  function removeGroup(id: string) { if (!canManageClientGroups) { notify('当前角色没有分组管理权限'); return; } if (clients.some(client => client.groupId === id)) { notify('该分组有客户信息，不能删除'); return; } if (clientSubgroups.some(subgroup => subgroup.groupId === id)) { notify('该分组有二级分类，不能删除'); return; } if (!confirm('确定删除该客户分类吗？')) return; const name = clientGroups.find(group => group.id === id)?.name ?? id; setClientGroups(current => current.filter(group => group.id !== id)); if (editingGroupId === id) { setEditingGroupId(null); setGroupName(''); } recordAudit('delete', '客户分组', `删除分组：${name}`, id); notify('客户分组已删除'); }
  function openSubgroupEditor(groupId: string, subgroup?: ClientSubgroup) { setSubgroupParentId(groupId); setEditingSubgroupId(subgroup?.id ?? null); setSubgroupName(subgroup?.name ?? ''); }
  ...
  function openRegionManager(level: RegionLevel) {
    ...
    const province = regionFilters.province || Object.keys(regionCatalog)[0] || '';
    const city = regionFilters.city || Object.keys(regionCatalog[province] || {})[0] || '';
    setRegionManager({ level, province, city });
    setRegionName('');
    setEditingRegionName(null);
  }
  function getManagedRegionItems(manager: { level: RegionLevel; province: string; city: string } | null) {
    if (!manager) return [];
    if (manager.level === 'province') return Object.keys(regionCatalog);
    if (manager.level === 'city') return Object.keys(regionCatalog[manager.province] || {});
    return regionCatalog[manager.province]?.[manager.city] || [];
  }
  function updateRegionManagerParent(field: 'province' | 'city', value: string) {
    if (!regionManager) return;
    if (field === 'province') {
      setRegionManager({ ...regionManager, province: value, city: Object.keys(regionCatalog[value] || {})[0] || '' });
    } else {
  ...
  function saveRegionCategory() {
    ...
    if (!name) { notify(`请输入${regionLevelLabels[regionManager.level]}名称`); return; }
    const items = getManagedRegionItems(regionManager);
    if (items.some(item => item !== editingRegionName && item === name)) { notify('分类名称已存在'); return; }
    ...
    setRegionCatalog(current => {
      ...
      if (regionManager.level === 'province') {
        if (oldName) { next[name] = next[oldName] || {}; delete next[oldName]; } else next[name] = {};
      } else if (regionManager.level === 'city') {
        next[regionManager.province] = { ...(next[regionManager.province] || {}) };
        if (oldName) { next[regionManager.province][name] = next[regionManager.province][oldName] || []; delete next[regionManager.province][oldName]; } else next[regionManager.province][name] = [];
      } else {
        next[regionManager.province] = { ...(next[regionManager.province] || {}) };
        next[regionManager.province][regionManager.city] = [...(next[regionManager.province][regionManager.city] || [])];
        if (oldName) next[regionManager.province][regionManager.city] = next[regionManager.province][regionManager.city].map(item => item === oldName ? name : item);
    ...
    if (oldName) {
      ...
      setClients(current => current.map(client => {
        const matches = regionManager.level === 'province' ? client.province === oldName : regionManager.level === 'city' ? client.province === managedProvince && client.city === oldName : client.province === managedProvince && client.city === managedCity && client.district === oldName;
        return matches ? { ...client, ...(regionManager.level === 'province' ? { province: name } : regionManager.level === 'city' ? { city: name } : { district: name }) } : client;
      }));
      setRegionFilters(current => ({
        ...current,
        ...(regionManager.level === 'province' && current.province === oldName ? { province: name, city: '', district: '' } : {}),
        ...(regionManager.level === 'city' && current.province === managedProvince && current.city === oldName ? { city: name, district: '' } : {}),
        ...(regionManager.level === 'district' && current.province === managedProvince && current.city === managedCity && current.district === oldName ? { district: name } : {}),
      }));
      setRegionManager(current => {
        if (!current) return current;
        if (regionManager.level === 'city' && current.level === 'city' && current.province === managedProvince && current.city === oldName) return { ...current, city: name };
        if (regionManager.level === 'province' && current.level === 'province' && current.province === oldName) return { ...current, province: name };
        return current;
      });
  ...
  function removeRegionCategory(name: string) {
    if (!regionManager || !canManageRegionLevel(regionManager.level)) return;
    const inUse = clients.some(client => regionManager.level === 'province' ? client.province === name : regionManager.level === 'city' ? client.province === regionManager.province && client.city === name : client.province === regionManager.province && client.city === regionManager.city && client.district === name);
    if (inUse) { notify('该地区分类已有客户使用，不能删除'); return; }
    if (!confirm(`确定删除${regionLevelLabels[regionManager.level]}“${name}”吗？`)) return;
    setRegionCatalog(current => {
      const next = JSON.parse(JSON.stringify(current)) as RegionCatalog;
      if (regionManager.level === 'province') delete next[name];
      else if (regionManager.level === 'city') { if (next[regionManager.province]) delete next[regionManager.province][name]; }
      else if (next[regionManager.province]?.[regionManager.city]) next[regionManager.province][regionManager.city] = next[regionManager.province][regionManager.city].filter(item => item !== name);
      return next;
    });
    if (editingRegionName === name) { setEditingRegionName(null); setRegionName(''); }
  ...
  function collectPendingAttachments(event: React.ChangeEvent<HTMLInputElement>, existingAttachments: Array<Attachment | PendingAttachment>) {
    const files = Array.from(event.target.files || []);
    event.target.value = '';
    if (!files.length) return [];
    const unsupported = files.find(file => !isSupportedAttachment(file));
    if (unsupported) { notify(`不支持上传 ${unsupported.name}，请选择图片、PDF、Word、Excel 或文本文件`); return []; }
  ...
  async function saveRecord() {
    ...
    try {
      ...
      if (recordModal === 'new') {
        ...
        setRecords(current => current.map(record => record.id === recordModal.id ? { ...record, ...details } : record));
        recordAudit('update', '费用明细', `修改费用明细：${selected.company}，单据ID ${recordModal.docId}`, recordModal.id);
        notify('费用明细已更新');
      }
  ...
  function openPaymentEdit(payment: PaymentItem) {
    ...
    setPendingPaymentAttachments([]);
    setSavedPaymentAttachments(Array.isArray(payment.attachments) ? payment.attachments : []);
    setPaymentAttachmentsToDelete([]);
    setPaymentModal(true);
  }
  ...
  function removeSavedPaymentAttachment(attachment: Attachment) {
    setSavedPaymentAttachments(current => current.filter(item => item.id !== attachment.id));
    setPaymentAttachmentsToDelete(current => current.some(item => item.id === attachment.id) ? current : [...current, attachment]);
  }
  ...
  async function removePayment(id: string) {
    ...
    setPayments(current => current.filter(payment => payment.id !== id));
      const deletionResults = await Promise.all((payment?.attachments || []).map(attachment => apiFetch(`/api/uploads/${encodeURIComponent(attachment.id)}`, { method: 'DELETE' }).catch(() => null)));
    recordAudit('delete', '回款明细', `删除回款：${selected?.company ?? ''}，单据ID ${payment?.docId ?? id}`, id);
  ...
  function openCostModal(cost: CostItem | 'new', docId?: string) {
    ...
    if (cost === 'new') {
      ...
      setFormCost({ docId: cost.docId, supplier: cost.supplier || suppliers[0] || '', reimburser: cost.reimburser || reimbursers[0] || '', costType: cost.costType || cost.feeTypes?.[0] || costTypes[0] || '', amount: String(cost.amount), note: cost.note, createdAt: toLocalDateTimeInputValue(cost.createdAt).slice(0, 10) });
      setSavedCostAttachments(Array.isArray(cost.attachments) ? cost.attachments : []);
    }
  ...
  function removeSavedCostAttachment(attachment: Attachment) {
    setSavedCostAttachments(current => current.filter(item => item.id !== attachment.id));
    setCostAttachmentsToDelete(current => current.some(item => item.id === attachment.id) ? current : [...current, attachment]);
  }
  async function saveCost() {
    ...
    try {
      ...
      if (costModal === 'new') {
        ...
        recordAudit('create', '成本明细', `新增成本：${selected.company}，单据ID ${record.docId}，${money(amount)}`, id);
        notify('成本已添加');
      } else if (costModal) {
        setCosts(current => current.map(cost => cost.id === costModal.id ? { ...cost, ...details } : cost));
        recordAudit('update', '成本明细', `修改成本：${selected.company}，单据ID ${record.docId}，${money(amount)}`, costModal.id);
        notify('成本明细已更新');
      }
  ...
  async function removeCost(id: string) {
    if (!canDeleteCost) { notify('当前角色没有删除成本费用权限'); return; }
    if (!confirm('确定删除这条成本明细吗？')) return;
    const cost = costs.find(item => item.id === id);
    setCosts(current => current.filter(item => item.id !== id));
      const deletionResults = await Promise.all((cost?.attachments || []).map(attachment => apiFetch(`/api/uploads/${encodeURIComponent(attachment.id)}`, { method: 'DELETE' }).catch(() => null)));
    recordAudit('delete', '成本明细', `删除成本：${selected?.company ?? ''}，单据ID ${cost?.docId ?? id}`, id);
  ...
  function openDailyExpenseModal(expense: DailyExpense | 'new') {
    ...
    if (expense === 'new') {
      setFormDailyExpense({ recordDate: toLocalDateTimeInputValue().slice(0, 10), expenseType: dailyExpenseTypes[0] || '', reimburser: reimbursers[0] || '', amount: '', note: '' });
      setSavedDailyExpenseAttachments([]);
    } else {
      setFormDailyExpense({ recordDate: expense.recordDate, expenseType: expense.expenseType, reimburser: expense.reimburser, amount: String(expense.amount), note: expense.note });
      setSavedDailyExpenseAttachments(Array.isArray(expense.attachments) ? expense.attachments : []);
    }
  ...
  function removeSavedDailyExpenseAttachment(attachment: Attachment) {
    ...
    setDailyExpenseAttachmentsToDelete(current => current.some(item => item.id === attachment.id) ? current : [...current, attachment]);
  }
  async function saveDailyExpense() {
    if (dailyExpenseModal === 'new' ? !canCreateDailyExpense : !canEditDailyExpense) {
      notify(dailyExpenseModal === 'new' ? '当前角色没有添加日常费用权限' : '当前角色没有修改日常费用权限');
      return;
    }
    ...
    try {
      ...
      if (dailyExpenseModal === 'new') {
        ...
        setDailyExpenses(current => [{ id, docId, ...details }, ...current]); recordAudit('create', '日常费用', `新增日常费用：${docId}，${money(amount)}`, id); notify('日常费用已添加');
  ...
  async function removeDailyExpense(id: string) {
    if (!canDeleteDailyExpense) { notify('当前角色没有删除日常费用权限'); return; }
    if (!confirm('确定删除这条日常费用吗？')) return;
    const expense = dailyExpenses.find(item => item.id === id); setDailyExpenses(current => current.filter(item => item.id !== id));
      const deletionResults = await Promise.all((expense?.attachments || []).map(attachment => apiFetch(`/api/uploads/${encodeURIComponent(attachment.id)}`, { method: 'DELETE' }).catch(() => null)));
    recordAudit('delete', '日常费用', `删除日常费用：${expense?.docId ?? id}`, id); notify(deletionResults.some(response => response && !response.ok && response.status !== 404) ? '日常费用已删除，但部分附件删除失败' : '日常费用已删除');
  }
  function saveDailyExpenseListValue(kind: 'type' | 'reimburser') {
    if (kind === 'type' ? !canManageDailyExpenseTypes : !canManageActiveReimbursers) {
      notify(kind === 'type' ? '当前角色没有费用类型管理权限' : '当前角色没有报销人管理权限');
      return;
    }
    const isType = kind === 'type'; const value = (isType ? dailyExpenseTypeName : reimburserName).trim(); const items = isType ? dailyExpenseTypes : reimbursers; const editing = isType ? editingDailyExpenseType : editingReimburser;
    if (!value || items.some(item => item === value && item !== editing)) { notify('请输入不重复的名称'); return; }
    if (editing) { const setter = isType ? setDailyExpenseTypes : setReimbursers; setter(current => sortTextValues(current.map(item => item === editing ? value : item))); recordAudit('update', isType ? '日常费用类型' : '报销人', `修改${isType ? '费用类型' : '报销人'}：${editing} -> ${value}`, editing); }
    else { const setter = isType ? setDailyExpenseTypes : setReimbursers; setter(current => sortTextValues([...current, value])); recordAudit('create', isType ? '日常费用类型' : '报销人', `新增${isType ? '费用类型' : '报销人'}：${value}`, value); }
    if (isType) { setDailyExpenseTypeName(''); setEditingDailyExpenseType(null); } else { setReimburserName(''); setEditingReimburser(null); }
  }
  function removeDailyExpenseListValue(kind: 'type' | 'reimburser', value: string) {
    if (kind === 'type' ? !canManageDailyExpenseTypes : !canManageActiveReimbursers) {
      notify(kind === 'type' ? '当前角色没有费用类型管理权限' : '当前角色没有报销人管理权限');
      return;
    }
    const isType = kind === 'type'; const used = dailyExpenses.some(item => (isType ? item.expenseType : item.reimburser) === value);
    if (used) { notify('该分类已有日常费用记录，不能删除'); return; }
    if (!confirm(`确定删除“${value}”吗？`)) return;
    (isType ? setDailyExpenseTypes : setReimbursers)(current => current.filter(item => item !== value)); recordAudit('delete', isType ? '日常费用类型' : '报销人', `删除${isType ? '费用类型' : '报销人'}：${value}`, value);
  }
  ...
  function saveCustomerInfo() {
    if (!selected || !formCustomerInfo.name.trim()) { notify('请填写运维资料名称'); return; }
    const details = { name: formCustomerInfo.name.trim(), note: formCustomerInfo.note.trim() };
    if (editingCustomerInfoId) {
      setCustomerInfos(current => current.map(info => info.id === editingCustomerInfoId ? { ...info, ...details } : info));
      recordAudit('update', '运维资料', `修改运维资料：${selected.company}，${details.name}`, editingCustomerInfoId);
      notify('运维资料已更新');
    } else {
      const id = 'ci' + Date.now();
      setCustomerInfos(current => [{ id, clientId: selected.id, ...details }, ...current]);
      recordAudit('create', '运维资料', `新增运维资料：${selected.company}，${details.name}`, id);
      notify('运维资料已添加');
    }
  ...
  function removeCustomerInfo(id: string) {
    ...
    recordAudit('delete', '运维资料', `删除运维资料：${selected?.company ?? ''}，${info?.name ?? id}`, id);
    notify('运维资料已删除');
  }
  async function removeRecord(id: string) {
    if (!canDeleteFee) { notify('当前角色没有删除记录权限'); return; }
    ...
    const deletionResults = await Promise.all(attachments.map(attachment => fetch(`/api/uploads/${encodeURIComponent(attachment.id)}`, { method: 'DELETE' }).catch(() => null)));
    recordAudit('delete', '费用明细', `删除费用明细：${selected?.company ?? ''}，单据ID ${record?.docId ?? id}`, id);
    notify(deletionResults.some(response => response && !response.ok && response.status !== 404) ? '记录已删除，但部分附件删除失败' : '记录已删除');
  }
  function addFeeType() {
    if (!canManageFeeTypes) { notify('当前角色没有费用类型管理权限'); return; }
  ...
  function saveFeeTypeEdit() {
    if (!canManageFeeTypes) { notify('当前角色没有费用类型管理权限'); return; }
    const name = feeTypeName.trim();
    if (!editingFeeType || !name || (feeTypes.includes(name) && name !== editingFeeType)) return;
    setFeeTypes(current => current.map(type => type === editingFeeType ? name : type));
    setRecords(current => current.map(record => record.feeType === editingFeeType ? { ...record, feeType: name } : record));
    recordAudit('update', '费用类型', `修改费用类型：${editingFeeType} -> ${name}`, editingFeeType);
    setFeeTypeName(''); setEditingFeeType(null); notify('费用类型已更新');
  }
  function removeFeeType(type: string) {
    if (!canManageFeeTypes) { notify('当前角色没有费用类型管理权限'); return; }
    if (records.some(record => record.feeType === type)) { notify('该费用类型已有记录，不能删除'); return; }
    if (!confirm('确定删除该费用类型吗？')) return;
    setFeeTypes(current => current.filter(item => item !== type));
    if (editingFeeType === type) { setEditingFeeType(null); setFeeTypeName(''); }
    recordAudit('delete', '费用类型', `删除费用类型：${type}`, type);
    notify('费用类型已删除');
  }
  function addEmployee() {
    if (!canManageEmployees) { notify('当前角色没有员工管理权限'); return; }
  ...
  function saveEmployeeEdit() {
    if (!canManageEmployees) { notify('当前角色没有员工管理权限'); return; }
    const name = employeeName.trim();
    if (!editingEmployee || !name || (employees.includes(name) && name !== editingEmployee)) return;
    setEmployees(current => sortTextValues(current.map(employee => employee === editingEmployee ? name : employee)));
    setRecords(current => current.map(record => record.employee === editingEmployee ? { ...record, employee: name } : record));
    recordAudit('update', '员工', `修改员工：${editingEmployee} -> ${name}`, editingEmployee);
    setEmployeeName(''); setEditingEmployee(null); notify('员工已更新');
  }
  function removeEmployee(employee: string) {
    if (!canManageEmployees) { notify('当前角色没有员工管理权限'); return; }
    if (records.some(record => record.employee === employee)) { notify('该员工已有费用明细，不能删除'); return; }
    if (!confirm('确定删除该员工吗？')) return;
    setEmployees(current => current.filter(item => item !== employee));
    if (editingEmployee === employee) { setEditingEmployee(null); setEmployeeName(''); }
    recordAudit('delete', '员工', `删除员工：${employee}`, employee);
    notify('员工已删除');
  }
  function addCostType() {
    if (!canManageCostTypes) { notify('当前角色没有成本明细费用类型管理权限'); return; }
  ...
  function saveCostTypeEdit() {
    if (!canManageCostTypes) { notify('当前角色没有成本明细费用类型管理权限'); return; }
    const name = costTypeName.trim();
    if (!editingCostType || !name || (costTypes.includes(name) && name !== editingCostType)) return;
    setCostTypes(current => current.map(type => type === editingCostType ? name : type));
    setCosts(current => current.map(cost => cost.costType === editingCostType ? { ...cost, costType: name } : cost));
    recordAudit('update', '利润分析表费用类型', `修改费用类型：${editingCostType} -> ${name}`, editingCostType);
    setCostTypeName(''); setEditingCostType(null); notify('成本明细费用类型已更新');
  }
  function removeCostType(type: string) {
    if (!canManageCostTypes) { notify('当前角色没有成本明细费用类型管理权限'); return; }
    if (costs.some(cost => cost.costType === type)) { notify('该费用类型已有利润分析记录，不能删除'); return; }
    if (!confirm('确定删除该成本明细费用类型吗？')) return;
    setCostTypes(current => current.filter(item => item !== type));
    if (editingCostType === type) { setEditingCostType(null); setCostTypeName(''); }
    recordAudit('delete', '成本明细费用类型', `删除费用类型：${type}`, type);
    notify('成本明细费用类型已删除');
  }
  function addSupplier() {
    if (!canManageCostSuppliers) { notify('当前角色没有供应商管理权限'); return; }
    const name = supplierName.trim();
    if (!name || suppliers.includes(name)) return;
    const id = `supplier-${Date.now()}`;
    setSuppliers(current => sortTextValues([...current, name]));
    setSupplierDetails(current => ({ ...current, [name]: { contact: supplierContact.trim(), phone: supplierPhone.trim() } }));
  ...
  function saveSupplierEdit() {
    if (!canManageCostSuppliers) { notify('当前角色没有供应商管理权限'); return; }
    const name = supplierName.trim();
    if (!editingSupplier || !name || (suppliers.includes(name) && name !== editingSupplier)) return;
    setSuppliers(current => sortTextValues(current.map(item => item === editingSupplier ? name : item)));
    setCosts(current => current.map(cost => cost.supplier === editingSupplier ? { ...cost, supplier: name } : cost));
    setSupplierDetails(current => {
      const next = { ...current };
      delete next[editingSupplier];
      next[name] = { contact: supplierContact.trim(), phone: supplierPhone.trim() };
      return next;
    });
    recordAudit('update', '供应商', `修改供应商：${editingSupplier} -> ${name}`, editingSupplier);
    setSupplierName(''); setSupplierContact(''); setSupplierPhone(''); setEditingSupplier(null); notify('供应商已更新');
  }
  function removeSupplier(name: string) {
    if (!canManageCostSuppliers) { notify('当前角色没有供应商管理权限'); return; }
    if (costs.some(cost => cost.supplier === name)) { notify('该供应商已有成本明细，不能删除'); return; }
    if (!confirm(`确定删除供应商“${name}”吗？`)) return;
    setSuppliers(current => current.filter(item => item !== name));
    setSupplierDetails(current => { const next = { ...current }; delete next[name]; return next; });
    if (editingSupplier === name) { setEditingSupplier(null); setSupplierName(''); setSupplierContact(''); setSupplierPhone(''); }
  ...
  function addPermissionGroup() {
    ...
    if (!name || permissionGroups.some(group => group.name === name)) { notify('请输入不重复的角色名称'); return; }
    const id = 'pg' + Date.now();
    setPermissionGroups(current => [...current, { id, name, permissions: permissionGroupPermissions }]);
  ...
  function savePermissionGroupEdit() {
    const name = permissionGroupName.trim();
    if (!editingPermissionGroupId || !name || permissionGroups.some(group => group.id !== editingPermissionGroupId && group.name === name)) { notify('请输入不重复的角色名称'); return; }
  ...
  function confirmPermissionGroupSave() {
    const name = permissionGroupName.trim();
    if (!editingPermissionGroupId || !name) { setPermissionSaveConfirmOpen(false); return; }
    setPermissionGroups(current => current.map(group => group.id === editingPermissionGroupId ? { ...group, name, permissions: permissionGroupPermissions } : group));
    recordAudit('update', '角色', `修改角色权限：${name}`, editingPermissionGroupId);
    setPermissionGroupName('');
  ...
  function removePermissionGroup(id: string) {
    if (users.some(user => user.permissionGroupId === id)) { notify('该角色已有成员，不能删除'); return; }
    if (!confirm('确定删除该角色吗？')) return;
    const group = permissionGroups.find(item => item.id === id);
    setPermissionGroups(current => current.filter(group => group.id !== id));
    recordAudit('delete', '角色', `删除角色：${group?.name ?? id}`, id);
    notify('角色已删除');
  }
  function openUserEdit(user: User) {
    setFormUser({ name: user.name, username: user.username || '', email: user.email, phone: user.phone || '', password: '', role: user.role });
    setUserModal(user);
  }
  function saveUser() {
    if (!formUser.name.trim() || !formUser.username.trim() || !formUser.email.trim() || !formUser.phone.trim()) { notify('请完整填写成员信息'); return; }
    if (userModal === 'new' && !formUser.password) { notify('请设置成员初始密码'); return; }
    if (formUser.password && !isPasswordValid(formUser.password)) { notify('密码至少8位，且需包含四类字符中的至少三类'); return; }
    const roleGroupId = permissionGroups.find(group => group.name === formUser.role)?.id;
    if (userModal === 'new') {
      const id = 'u' + Date.now();
      setUsers(current => [{ id, ...formUser, mustChangePassword: mustChangeOnFirstLogin, permissionGroupId: roleGroupId, status: '正常' }, ...current]);
      recordAudit('create', '成员', `新增成员：${formUser.name}（${formUser.username}）`, id);
      notify('成员已添加');
    } else if (userModal) {
      setUsers(current => current.map(user => user.id === userModal.id ? { ...user, ...formUser, mustChangePassword: formUser.password ? true : user.mustChangePassword, permissionGroupId: roleGroupId ?? user.permissionGroupId } : user));
      recordAudit('update', '成员', `修改成员：${formUser.name}（${formUser.username}）`, userModal.id);
      notify('成员资料已更新');
    }
    setUserModal(null);
    setFormUser({ name: '', username: '', email: '', phone: '', password: '', role: '财务' });
  }
  function saveOwnPassword() {
    if (!isPasswordValid(newPassword)) { notify('密码至少8位，且需包含大写、小写、数字、特殊符号中的至少三类'); return; }
    if (newPassword !== confirmPassword) { notify('两次输入的密码不一致'); return; }
    if (!session) return;
    void apiFetch('/api/auth/password', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ password: newPassword }) }).then(async response => {
      if (!response.ok) { notify('密码更新失败，请稍后重试'); return; }
      const result = await response.json() as { user?: User };
      if (result.user) { setUsers(current => current.map(user => user.id === result.user?.id ? result.user as User : user)); setSession(result.user); }
      recordAudit('security', '密码', '用户修改了自己的登录密码', session.id);
      setPasswordModal(false); setNewPassword(''); setConfirmPassword(''); notify('密码已更新');
    }).catch(() => notify('密码服务暂时不可用'));
  }
  function resetUserPassword(user: User) {
    if (session?.role !== '管理员') return;
    const password = window.prompt(`请输入${user.name}的新密码（至少8位，含三类字符）`, 'TempA1!change');
    if (!password) return;
    if (!isPasswordValid(password)) { notify('密码至少8位，且需包含三类字符'); return; }
    void apiFetch(`/api/users/${encodeURIComponent(user.id)}/reset-password`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ password }) }).then(async response => {
      if (!response.ok) { notify('密码重置失败'); return; }
      const result = await response.json() as { user?: User };
      if (result.user) setUsers(current => current.map(item => item.id === result.user?.id ? result.user as User : item));
      recordAudit('security', '密码', `管理员重置成员密码：${user.name}`, user.id); notify('密码已重置，用户下次登录需修改密码');
    }).catch(() => notify('密码服务暂时不可用'));
  }
  ...
  function renderClientGroup(group: ClientGroup) {
    ...
    const renderClient = (client: Client) => <button className={'sub-client ' + (selected?.id === client.id && active === 'clients' ? 'active' : '')} key={client.id} onClick={() => { setSelectedId(client.id); setActive('clients'); }}><span><strong>{client.company}</strong></span></button>;
    return <div className="client-group" key={group.id}><button className="group-label" onClick={() => { setExpandedGroups(current => expanded ? current.filter(id => id !== group.id) : [...current, group.id]); if (expanded) setExpandedSubgroups(current => current.filter(id => !subgroups.some(subgroup => subgroup.id === id))); }} aria-expanded={expanded}>{group.name}<span>{groupClients.length}</span></button>{expanded && (subgroups.length ? subgroups.map(subgroup => { const subgroupClients = sortClientsByNameLength(groupClients.filter(client => client.subgroupId === subgroup.id)); const subgroupExpanded = searchActive || expandedSubgroups.includes(subgroup.id); return <div className="sub-client-group" key={subgroup.id}><button className="sub-client-group-label" onClick={() => setExpandedSubgroups(current => subgroupExpanded ? current.filter(id => id !== subgroup.id) : [...current, subgroup.id])} aria-expanded={subgroupExpanded}>{subgroup.name}<span>{subgroupClients.length}</span></button>{subgroupExpanded && subgroupClients.map(renderClient)}</div>; }) : sortClientsByNameLength(groupClients).map(renderClient))}</div>;
  }
  function renderNestedRegionDirectory() {
    const typeId = (client: Client) => client.customerTypeId || client.groupId;
    const toggle = (key: string) => setExpandedRegionLevels(current => current.includes(key) ? current.filter(item => item !== key) : [...current, key]);
    const expanded = (key: string) => searchActive || expandedRegionLevels.includes(key);
    const levelButton = (key: string, label: string, count: number, className: string) => <button className={className} onClick={() => toggle(key)} aria-expanded={expanded(key)}><span className="directory-collapse-icon"><ChevronDown size={14} /></span><strong>{label}</strong><span>{count} 位客户</span></button>;
    const provinces = [...new Set(filteredClients.map(client => client.province || '未设置省份'))].sort((a, b) => a.localeCompare(b, 'zh-CN'));
    return <div className="client-directory region-directory panel"><div className="directory-head"><div><h3>客户目录</h3><span>省份/直辖市 → 地市 → 区/县 → 客户类型 → 客户名称</span></div><span>{filteredClients.length} 位客户</span></div><div className="region-tree">{provinces.map(province => { const provinceClients = filteredClients.filter(client => (client.province || '未设置省份') === province); const provinceKey = `province:${province}`; return <section className="directory-region-level" key={provinceKey}>{levelButton(provinceKey, province, provinceClients.length, 'directory-group-head')}{expanded(provinceKey) && <div className="region-tree-children">{[...new Set(provinceClients.map(client => client.city || '未设置地市'))].sort((a, b) => a.localeCompare(b, 'zh-CN')).map(city => { const cityClients = provinceClients.filter(client => (client.city || '未设置地市') === city); const cityKey = `${provinceKey}/city:${city}`; return <div className="directory-city-level" key={cityKey}>{levelButton(cityKey, city, cityClients.length, 'directory-subgroup-head')}{expanded(cityKey) && <div className="region-tree-children">{[...new Set(cityClients.map(client => client.district || '未设置区县'))].sort((a, b) => a.localeCompare(b, 'zh-CN')).map(district => { const districtClients = cityClients.filter(client => (client.district || '未设置区县') === district); const districtKey = `${cityKey}/district:${district}`; return <div className="directory-district-level" key={districtKey}>{levelButton(districtKey, district, districtClients.length, 'directory-subgroup-head')}{expanded(districtKey) && <div className="region-tree-children region-type-children">{clientGroups.map(type => { const typeClients = sortClientsByNameLength(districtClients.filter(client => typeId(client) === type.id)); if (!typeClients.length) return null; const typeKey = `${districtKey}/type:${type.id}`; return <div className="directory-type-level" key={typeKey}>{levelButton(typeKey, type.name, typeClients.length, 'directory-type-head')}{expanded(typeKey) && <div className="directory-client-grid">{typeClients.map(client => <div className="directory-client" key={client.id}><button className="directory-client-main" onClick={() => { setSelectedId(client.id); setActive('clients'); }}><span className="client-avatar">{client.name.slice(0, 1)}</span><span><strong>{client.company}</strong><small>{client.name} · {client.phone || '未设置电话'}</small></span></button><div className="directory-client-actions"><button className="icon-btn" onClick={() => openClient(client)} disabled={!canEditClient} aria-label={`编辑${client.name}`} title="编辑客户"><Edit3 size={15} /></button><button className="icon-btn danger" onClick={() => removeClient(client.id)} disabled={!canDeleteClient} aria-label={`删除${client.name}`} title="删除客户"><Trash2 size={15} /></button></div></div>)}</div>}</div>; })}</div>}</div>; })}</div>}</div>; })}</div>}</section>; })}{!filteredClients.length && <div className="empty">暂无匹配客户</div>}</div></div>;
  }
  function renderRegionDirectoryFilters() {
    const updateRegionFilter = (key: keyof typeof regionFilters, value: string) => {
      if (key === 'province') setRegionFilters({ province: value, city: '', district: '', customerTypeId: '' });
      else if (key === 'city') setRegionFilters(current => ({ ...current, city: value, district: '', customerTypeId: '' }));
      else if (key === 'district') setRegionFilters(current => ({ ...current, district: value, customerTypeId: '' }));
      else setRegionFilters(current => ({ ...current, customerTypeId: value }));
    };
    const filterSelect = (label: string, placeholder: string, value: string, options: string[], key: keyof typeof regionFilters, level: RegionLevel) => <label className="region-filter"><span className="region-filter-label"><span>{label}</span><button type="button" className="region-manage-btn" onClick={event => { event.preventDefault(); event.stopPropagation(); openRegionManager(level); }} disabled={!canManageRegionLevel(level)} aria-label={`管理${label}分类`} title={`管理${label}分类`}><Settings size={13} /></button></span><select value={value} onChange={event => updateRegionFilter(key, event.target.value)}><option value="">{placeholder}</option>{options.map(option => <option key={option} value={option}>{option}</option>)}</select></label>;
    const hasRegionFilters = Object.values(regionFilters).some(Boolean);
    return <div className="client-directory region-directory panel"><div className="directory-head directory-filter-head"><div><h3>客户目录</h3><span>按地区与客户类型快速筛选客户</span></div><div className="directory-head-actions"><span>{filteredClients.length} 位客户</span><button className="text-btn" onClick={() => setRegionFilters({ province: '', city: '', district: '', customerTypeId: '' })} disabled={!hasRegionFilters}>清除筛选</button></div></div><div className="region-filter-bar">{filterSelect('省份/直辖市', '请选择省份/直辖市', regionFilters.province, regionProvinceOptions, 'province', 'province')}{filterSelect('地市', '请选择地市', regionFilters.city, regionCityOptions, 'city', 'city')}{filterSelect('区/县', '请选择区/县', regionFilters.district, regionDistrictOptions, 'district', 'district')}<label className="region-filter"><span className="region-filter-label"><span>客户类型</span><button type="button" className="region-manage-btn" onClick={event => { event.preventDefault(); event.stopPropagation(); openGroupManager(); }} disabled={!canManageClientGroups} aria-label="管理客户类型分类" title="管理客户类型分类"><Settings size={13} /></button></span><select value={regionFilters.customerTypeId} onChange={event => updateRegionFilter('customerTypeId', event.target.value)}><option value="">请选择客户类型</option>{regionTypeOptions.map(type => <option key={type.id} value={type.id}>{type.name}</option>)}</select></label><span className="region-filter-count">当前显示 {filteredClients.length} 位</span></div><div className="region-client-grid">{clientPageEntries.map(client => { const type = clientGroups.find(group => group.id === (client.customerTypeId || client.groupId))?.name || '未设置客户类型'; return <div className="directory-client" key={client.id}><button className="directory-client-main" onClick={() => { setSelectedId(client.id); setActive('clients'); }}><span className="client-avatar">{client.name.slice(0, 1)}</span><span><strong>{client.company}</strong><small>{client.name} · {client.phone || '未设置电话'}</small><small>{client.province || '未设置省份'} · {client.city || '未设置地市'} · {client.district || '未设置区县'} · {type}</small></span></button><div className="directory-client-actions"><button className="icon-btn" onClick={() => { setSelectedId(client.id); setActive('clients'); }} aria-label={`查看${client.name}`} title="查看客户详情"><ChevronRight size={15} /></button><button className="icon-btn" onClick={() => openClient(client)} disabled={!canEditClient} aria-label={`编辑${client.name}`} title="编辑客户"><Edit3 size={15} /></button><button className="icon-btn danger" onClick={() => removeClient(client.id)} disabled={!canDeleteClient} aria-label={`删除${client.name}`} title="删除客户"><Trash2 size={15} /></button></div></div>; })}{!filteredClients.length && <div className="empty">暂无匹配客户</div>}</div>{renderDashboardPagination({ total: filteredClients.length, page: clientPage, pageSize: clientPageSize, pageCount: clientPageCount, onPageChange: setClientPage, onPageSizeChange: setClientPageSize })}</div>;
  }
  function renderRegionDirectory() {
    ...
    */
    const rows = [...filteredClients].sort((a, b) => {
      const left = [a.province || '未设置省份', a.city || '未设置地市', a.district || '未设置区县', a.company].join('');
      const right = [b.province || '未设置省份', b.city || '未设置地市', b.district || '未设置区县', b.company].join('');
      return left.localeCompare(right, 'zh-CN');
    });
    return <div className="client-directory region-directory panel"><div className="directory-head"><div><h3>客户目录</h3><span>省份/直辖市 → 地市 → 区/县 → 客户类型 → 客户名称</span></div><span>{filteredClients.length} 位客户</span></div><div className="region-client-grid">{rows.map(client => { const type = clientGroups.find(group => group.id === (client.customerTypeId || client.groupId))?.name || '未设置客户类型'; return <div className="region-client-row" key={client.id}><div className="region-breadcrumb"><span>{client.province || '未设置省份'}</span><ChevronRight size={13} /><span>{client.city || '未设置地市'}</span><ChevronRight size={13} /><span>{client.district || '未设置区县'}</span><ChevronRight size={13} /><strong>{type}</strong></div><div className="directory-client"><button className="directory-client-main" onClick={() => { setSelectedId(client.id); setActive('clients'); }}><span className="client-avatar">{client.name.slice(0, 1)}</span><span><strong>{client.company}</strong><small>{client.name} · {client.phone || '未设置电话'}</small></span></button><div className="directory-client-actions"><button className="icon-btn" onClick={() => openClient(client)} disabled={!canEditClient} aria-label={`编辑${client.name}`} title="编辑客户"><Edit3 size={15} /></button><button className="icon-btn danger" onClick={() => removeClient(client.id)} disabled={!canDeleteClient} aria-label={`删除${client.name}`} title="删除客户"><Trash2 size={15} /></button></div></div></div>; })}</div>{!rows.length && <div className="empty">暂无匹配客户</div>}</div>;
  }
  const permissionHierarchy: { parent: PermissionKey; children: PermissionKey[] }[] = [
  ...
  const renderPermissionNode = (permission: { key: PermissionKey; label: string }) => {
    const hierarchy = permissionHierarchy.find(item => item.parent === permission.key);
    if (!hierarchy) return <label key={permission.key}><input type="checkbox" checked={permissionGroupPermissions.includes(permission.key)} onChange={event => setPermissionEnabled(permission.key, event.target.checked)} />{permission.label}</label>;
    return <div className="permission-client-group" key={permission.key}><label><input type="checkbox" checked={permissionGroupPermissions.includes(permission.key) || hierarchy.children.some(key => permissionGroupPermissions.includes(key))} onChange={event => setPermissionEnabled(permission.key, event.target.checked)} />{permission.label}</label><div className="permission-subitems">{allPermissions.filter(item => hierarchy.children.includes(item.key)).map(renderPermissionNode)}</div></div>;
  };
  const renderPermissionItem = (permission: { key: PermissionKey; label: string }) => {
    return renderPermissionNode(permission);
  };
  const sortedUsers = [...users].sort((a, b) => Number(b.role === '管理员') - Number(a.role === '管理员'));
  const renderAuditRows = (entries: AuditLog[]) => entries.map(log => <div className="audit-grid" key={log.id}><time dateTime={log.createdAt}>{new Date(log.createdAt).toLocaleString('zh-CN', { hour12: false })}</time><span>{log.username}</span><span className={`audit-action audit-${log.action}`}>{({ create: '新增', update: '修改', delete: '删除', security: '安全操作', send: '发送' } as Record<AuditAction, string>)[log.action]}</span><span>{log.entity}</span><span className="audit-summary">{log.summary}</span></div>);
  const renderAuditPagination = () => <div className="audit-pagination"><span>共 {auditSearchEntries.length} 条{auditSearch.trim() ? `（全部 ${auditLogs.length} 条）` : ''}</span><label>每页<select value={auditPageSize} onChange={event => { setAuditPageSize(Number(event.target.value)); setAuditPage(1); }}>{[10, 20, 30, 50, 100].map(size => <option key={size} value={size}>{size}</option>)}</select>条</label><button className="icon-btn" onClick={() => setAuditPage(page => Math.max(1, page - 1))} disabled={auditPage <= 1} aria-label="上一页" title="上一页"><ArrowLeft size={15} /></button><div className="audit-page-numbers">{auditPageNumbers.map((page, index) => <span key={page}>{index > 0 && page - auditPageNumbers[index - 1] > 1 && <i>...</i>}<button className={page === auditPage ? 'active' : ''} onClick={() => setAuditPage(page)}>{page}</button></span>)}</div><button className="icon-btn" onClick={() => setAuditPage(page => Math.min(auditPageCount, page + 1))} disabled={auditPage >= auditPageCount} aria-label="下一页" title="下一页"><ChevronRight size={15} /></button><label className="audit-jump">跳转<input type="number" min="1" max={auditPageCount} value={auditPage} onChange={event => setAuditPage(Math.min(auditPageCount, Math.max(1, Number(event.target.value) || 1)))} />页</label></div>;
  const renderDashboardPagination = ({ total, page, pageSize, pageCount, onPageChange, onPageSizeChange }: { total: number; page: number; pageSize: number; pageCount: number; onPageChange: (page: number) => void; onPageSizeChange: (size: number) => void }) => {
    const pageNumbers = Array.from({ length: pageCount }, (_, index) => index + 1).filter(item => pageCount <= 7 || item === 1 || item === pageCount || Math.abs(item - page) <= 1);
    return <div className="dashboard-pagination"><span>共 {total} 条</span><label>每页<select value={pageSize} onChange={event => { onPageSizeChange(Number(event.target.value)); onPageChange(1); }}>{[10, 20, 30, 50, 100].map(size => <option key={size} value={size}>{size}</option>)}</select>条</label><button className="icon-btn" onClick={() => onPageChange(Math.max(1, page - 1))} disabled={page <= 1} aria-label="上一页" title="上一页"><ArrowLeft size={15} /></button><div className="dashboard-page-numbers">{pageNumbers.map((item, index) => <span key={item}>{index > 0 && item - pageNumbers[index - 1] > 1 && <i>...</i>}<button className={item === page ? 'active' : ''} onClick={() => onPageChange(item)}>{item}</button></span>)}</div><button className="icon-btn" onClick={() => onPageChange(Math.min(pageCount, page + 1))} disabled={page >= pageCount} aria-label="下一页" title="下一页"><ChevronRight size={15} /></button><label className="dashboard-jump">跳转<input type="number" min="1" max={pageCount} value={page} onChange={event => onPageChange(Math.min(pageCount, Math.max(1, Number(event.target.value) || 1)))} />页</label></div>;
  };
  const renderCompanyExpenseAnalysisPanel = () => <section className={'panel dashboard-panel company-expense-analysis-panel ' + (companyExpenseCollapsed ? 'is-collapsed' : '')}>
    <div className="panel-head">
      <button className="panel-collapse-head" onClick={() => setCompanyExpenseCollapsed(current => !current)} aria-expanded={!companyExpenseCollapsed}>
        <div><h3>费用统计</h3><span>按时间、费用类型和报销人汇总日常费用 · 共 {companyExpenseRows.length} 条</span></div><ChevronDown size={17} />
      </button>
      <Banknote size={18} aria-hidden="true" />
    </div>
    <div className="profit-analysis-body company-expense-analysis-body">
      <div className="profit-filter-bar company-expense-filter-bar" aria-label="费用统计筛选条件">
        <label>开始时间<input type="date" value={companyExpenseFilters.startDate} onChange={event => setCompanyExpenseFilters(current => ({ ...current, startDate: event.target.value }))} /></label>
        <label>结束时间<input type="date" min={companyExpenseFilters.startDate || undefined} value={companyExpenseFilters.endDate} onChange={event => setCompanyExpenseFilters(current => ({ ...current, endDate: event.target.value }))} /></label>
        <label>费用类型<select value={companyExpenseFilters.expenseType} onChange={event => setCompanyExpenseFilters(current => ({ ...current, expenseType: event.target.value }))}><option value="">全部费用类型</option>{dailyExpenseTypes.map(type => <option key={type} value={type}>{type}</option>)}</select></label>
        <label>报销人<select value={companyExpenseFilters.reimburser} onChange={event => setCompanyExpenseFilters(current => ({ ...current, reimburser: event.target.value }))}><option value="">全部报销人</option>{reimbursers.map(reimburser => <option key={reimburser} value={reimburser}>{reimburser}</option>)}</select></label>
        <button className="text-btn profit-filter-reset" onClick={() => setCompanyExpenseFilters({ startDate: '', endDate: '', expenseType: '', reimburser: '' })} disabled={!Object.values(companyExpenseFilters).some(Boolean)}>清除筛选</button>
      </div>
      <div className="profit-summary-grid company-expense-summary-grid">
        <div><span>统计费用</span><strong className="red">{money(companyExpenseTotal)}</strong></div>
        <div><span>费用笔数</span><strong>{companyExpenseRows.length}</strong></div>
        <div><span>平均费用</span><strong>{money(companyExpenseAverage)}</strong></div>
      </div>
      <div className="profit-analysis-table company-expense-analysis-table">
        <div className="profit-analysis-row profit-analysis-head"><span>时间</span><span>费用类型</span><span>报销人</span><span>费用单据</span><span>成本费用</span><span>备注</span></div>
        {companyExpensePageEntries.map(expense => <div className="profit-analysis-row company-expense-analysis-row" key={expense.id}><span data-label="时间">{expense.recordDate}</span><span data-label="费用类型">{expense.expenseType}</span><span data-label="报销人">{expense.reimburser}</span><strong data-label="费用单据">{expense.docId}</strong><strong className="red" data-label="成本费用">{money(expense.amount)}</strong><span data-label="备注">{expense.note || '无备注'}</span></div>)}
        {!companyExpenseRows.length && <div className="empty">暂无符合筛选条件的费用记录</div>}
      </div>
      {companyExpenseRows.length > 0 && renderDashboardPagination({ total: companyExpenseRows.length, page: companyExpensePage, pageSize: companyExpensePageSize, pageCount: companyExpensePageCount, onPageChange: setCompanyExpensePage, onPageSizeChange: setCompanyExpensePageSize })}
    </div>
  </section>;
  const renderProfitAnalysisPanel = () => <section className={'panel dashboard-panel profit-analysis-panel ' + (profitAnalysisCollapsed ? 'is-collapsed' : '')}>
    <div className="panel-head">
      <button className="panel-collapse-head" onClick={() => setProfitAnalysisCollapsed(current => !current)} aria-expanded={!profitAnalysisCollapsed}>
        <div><h3>利润分析</h3><span>按费用单据汇总营收、成本、利润与欠款 · 共 {profitAnalysisRows.length} 条</span></div><ChevronDown size={17} />
      </button>
      <TrendingUp size={18} aria-hidden="true" />
    </div>
    <div className="profit-analysis-body">
      <div className="profit-filter-bar" aria-label="利润分析筛选条件">
        <label>开始日期<input type="date" value={profitFilters.startDate} onChange={event => setProfitFilters(current => ({ ...current, startDate: event.target.value }))} /></label>
        <label>结束日期<input type="date" min={profitFilters.startDate || undefined} value={profitFilters.endDate} onChange={event => setProfitFilters(current => ({ ...current, endDate: event.target.value }))} /></label>
        <label>业务员<select value={profitFilters.employee} onChange={event => setProfitFilters(current => ({ ...current, employee: event.target.value, docId: '' }))}><option value="">全部业务员</option>{profitAnalysisEmployees.map(employee => <option key={employee} value={employee}>{employee}</option>)}</select></label>
        <label>客户<select value={profitFilters.clientId} onChange={event => setProfitFilters(current => ({ ...current, clientId: event.target.value, docId: '' }))}><option value="">全部客户</option>{profitAnalysisClients.map(client => <option key={client.id} value={client.id}>{client.company}</option>)}</select></label>
        <label>客户费用单据<select value={profitFilters.docId} onChange={event => setProfitFilters(current => ({ ...current, docId: event.target.value }))}><option value="">全部费用单据</option>{profitAnalysisDocumentOptions.map(record => <option key={record.id} value={record.docId}>{record.docId} · {clients.find(client => client.id === record.clientId)?.company || '未知客户'}</option>)}</select></label>
        <button className="text-btn profit-filter-reset" onClick={() => setProfitFilters({ startDate: '', endDate: '', employee: '', clientId: '', docId: '' })} disabled={!Object.values(profitFilters).some(Boolean)}>清除筛选</button>
      </div>
      <div className="profit-summary-grid">
        <div><span>营收</span><strong>{money(profitAnalysisSummary.revenue)}</strong></div>
        <div><span>成本</span><strong className="red">{money(profitAnalysisSummary.cost)}</strong></div>
        <div><span>利润</span><strong className={profitAnalysisSummary.profit >= 0 ? 'green' : 'red'}>{money(profitAnalysisSummary.profit)}</strong></div>
        <div><span>欠款</span><strong className={profitAnalysisSummary.arrears ? 'red' : 'green'}>{money(profitAnalysisSummary.arrears)}</strong></div>
      </div>
      <div className="profit-analysis-table">
        <div className="profit-analysis-row profit-analysis-head"><span>时间</span><span>业务员</span><span>客户</span><span>费用单据</span><span>营收</span><span>成本</span><span>利润</span><span>欠款</span></div>
        {profitAnalysisPageEntries.map(row => <div className="profit-analysis-row" key={row.id}><span data-label="时间">{row.date}</span><span data-label="业务员">{row.employee}</span><span data-label="客户">{row.client}</span><strong data-label="费用单据">{row.docId}</strong><strong data-label="营收">{money(row.revenue)}</strong><strong className="red" data-label="成本">{money(row.cost)}</strong><strong className={row.profit >= 0 ? 'green' : 'red'} data-label="利润">{money(row.profit)}</strong><strong className={row.arrears ? 'red' : 'green'} data-label="欠款">{money(row.arrears)}</strong></div>)}
        {!profitAnalysisRows.length && <div className="empty">暂无符合筛选条件的费用单据</div>}
      </div>
  ...
  function renderCostRows(record: RecordItem) {
    ...
    return rows.map((cost, index) => {
     ...
     return <div className="app-shell"><aside className="app-sidebar"><div className="sidebar-brand"><div className="brand-mark"><CircleDollarSign size={20} /></div><div><strong>OA帮</strong><span>费用客户管理系统</span></div></div><nav>{canViewDashboard && <button className={active === 'dashboard' ? 'nav-item active' : 'nav-item'} onClick={() => setActive('dashboard')}><LayoutDashboard size={17} />总览</button>}{canViewClients && <div className="nav-group"><button className={active === 'clients' ? 'nav-item active' : 'nav-item'} onClick={() => { const alreadyOnClients = active === 'clients'; setActive('clients'); setClientsExpanded(current => alreadyOnClients ? !current : false); }} aria-expanded={clientsExpanded}><Users size={17} />客户列表</button>{clientsExpanded && <div className="sub-nav">{clientGroups.map(renderClientGroup)}{!filteredClients.length && <div className="sub-empty">暂无客户</div>}</div>}</div>}<div className="nav-group"><button className={active === 'companyExpenses' || active === 'dailyExpenses' ? 'nav-item active' : 'nav-item'} onClick={() => { setActive('companyExpenses'); setCompanyExpensesExpanded(current => !current); }} aria-expanded={companyExpensesExpanded}><Banknote size={17} />公司费用管理</button><div className="sub-nav"><button className={'sub-nav-item ' + (active === 'dailyExpenses' ? 'active' : '')} onClick={() => { setActive('dailyExpenses'); setCompanyExpensesExpanded(true); }}>日常费用管理</button></div></div>{canViewUsers && <button className={active === 'users' ? 'nav-item active' : 'nav-item'} onClick={() => setActive('users')}><Settings size={17} />权限管理</button>}</nav><div className="sidebar-bottom"><div className="profile"><div className="avatar">{session.name.slice(0, 1)}</div><div><strong>{session.name}</strong><span>{session.role}</span></div></div><button className="logout" onClick={() => setSession(null)}><LogOut size={16} />退出登录</button></div></aside><main className="main"><header className="topbar"><button className="mobile-menu icon-btn" aria-label="菜单"><Menu size={19} /></button><div><p className="eyebrow">{active === 'dashboard' ? 'OVERVIEW' : active === 'clients' ? 'CLIENTS' : active === 'companyExpenses' ? 'COMPANY EXPENSES' : active === 'dailyExpenses' ? 'DAILY EXPENSES' : 'ACCESS CONTROL'}</p><h2>{active === 'dashboard' ? '业务总览' : active === 'clients' ? '客户列表' : active === 'companyExpenses' ? '公司费用管理' : active === 'dailyExpenses' ? '日常费用管理' : '权限管理'}</h2></div><div className="top-actions"><span className="today"><CalendarDays size={15} />2026年8月21日</span><div className="account-menu" onMouseEnter={handleAccountMenuEnter} onMouseLeave={handleAccountMenuLeave}><button className="top-avatar" onClick={toggleAccountMenu} aria-label="账户菜单" aria-expanded={accountMenuOpen}>{session.name.slice(0, 1)}</button>{accountMenuOpen && <div className="account-menu-popover"><div className="account-menu-user"><strong>{session.username || session.name}</strong><span>{session.role}</span></div><div className="account-menu-divider" /><button onClick={() => { setNewPassword(''); setConfirmPassword(''); closeAccountMenu(); setPasswordModal(true); }}><ShieldCheck size={16} />修改密码</button><button className="account-menu-logout" onClick={() => { closeAccountMenu(); setSession(null); }}><LogOut size={16} />退出登录</button></div>}</div></div></header>
      ...
      {active === 'companyExpenses' && canViewCompanyExpenses && renderCompanyExpenseAnalysisPanel()}
      {active === 'dailyExpenses' && canViewDailyExpenses && <section className="daily-expenses-page">
         <div className="panel-head daily-expenses-head"><div><h3>日常费用管理</h3><span>记录公司日常报销支出与费用凭证</span></div><div className="toolbar-actions"><button className="secondary-btn small" onClick={() => { closeReimburserManager(); setDailyExpenseTypeManagerOpen(true); }} disabled={!canManageDailyExpenseTypes}><Settings size={15} />费用类型</button><button className="secondary-btn small" onClick={() => { closeDailyExpenseTypeManager(); setReimburserManagerScope('daily'); setReimburserManagerOpen(true); }} disabled={!canManageDailyExpenseReimbursers}><Settings size={15} />报销人</button><button className="primary-btn small" onClick={() => openDailyExpenseModal('new')} disabled={!canCreateDailyExpense}><Plus size={16} />添加日常费用</button></div></div>
         <div className="daily-expenses-table"><div className="daily-expense-row daily-expense-row-head"><span>费用单据ID</span><span>时间</span><span>费用类型</span><span>报销人</span><span>成本费用（元）</span><span>备注</span><span>附件</span><span>操作</span></div>{dailyExpensePageEntries.map(expense => <div className="daily-expense-row" key={expense.id}><strong>{expense.docId}</strong><span>{expense.recordDate}</span><span>{expense.expenseType}</span><span>{expense.reimburser}</span><strong>{money(expense.amount)}</strong><span className="daily-expense-note">{expense.note}</span><span>{expense.attachments?.length || 0} 个</span><div className="record-actions"><button className="icon-btn" onClick={() => openDailyExpenseModal(expense)} disabled={!canEditDailyExpense} aria-label={`编辑${expense.docId}`} title="编辑日常费用"><Edit3 size={15} /></button><button className="icon-btn danger" onClick={() => removeDailyExpense(expense.id)} disabled={!canDeleteDailyExpense} aria-label={`删除${expense.docId}`} title="删除日常费用"><Trash2 size={15} /></button></div></div>)}{!dailyExpenseRows.length && <div className="empty">暂无日常费用记录</div>}</div>
        {dailyExpenseRows.length > 0 && <div className="daily-expense-pagination">{renderDashboardPagination({ total: dailyExpenseRows.length, page: dailyExpensePage, pageSize: dailyExpensePageSize, pageCount: dailyExpensePageCount, onPageChange: setDailyExpensePage, onPageSizeChange: setDailyExpensePageSize })}</div>}
      </section>}
      {active === 'clients' && <section className="clients-page">
       {!selected && <div className="page-toolbar"><div><p className="eyebrow">CLIENT DIRECTORY</p><div className="client-context">客户列表</div><span className="client-list-description">未选择分类时显示全部客户，选择后按当前分类筛选</span></div><div className="toolbar-actions"><div className="search-wrap"><Search size={17} /><input placeholder="搜索客户、地区或客户类型" value={search} onChange={e => setSearch(e.target.value)} /></div><button className="primary-btn" onClick={() => openClient('new')} disabled={!canCreateClient}><Plus size={17} />添加客户</button></div></div>}
 {!selected && renderRegionDirectoryFilters()}
 {!selected && <div className={'panel dashboard-panel latest-clients-panel ' + (recentClientsCollapsed ? 'is-collapsed' : '')}><div className="panel-head"><button className="panel-collapse-head" onClick={() => setRecentClientsCollapsed(current => !current)} aria-expanded={!recentClientsCollapsed}><div><h3>最近客户</h3><span>按增加时间倒序 · 最近 {latestClients.length} / {clients.length} 位</span></div><ChevronDown size={17} /></button><span className="latest-clients-count">共 {latestClients.length} 位</span></div><div className="dashboard-panel-body"><div className="dashboard-client-grid">{recentClientsPageEntries.map(c => <button className="dashboard-client-card" key={c.id} onClick={() => setSelectedId(c.id)}><span className="client-avatar">{c.name.slice(0, 1)}</span><span className="dashboard-client-copy"><strong>{c.company}</strong><small>{c.name} · {c.phone}</small><small>增加时间 {formatClientCreatedAt(c.createdAt)}</small></span><ChevronRight size={16} /></button>)}{!latestClients.length && <div className="empty">暂无客户记录</div>}</div>{renderDashboardPagination({ total: latestClients.length, page: recentClientsPage, pageSize: recentClientsPageSize, pageCount: recentClientsPageCount, onPageChange: setRecentClientsPage, onPageSizeChange: setRecentClientsPageSize })}</div></div>}
{false && <div className="client-directory panel">{clientGroups.filter(group => !searchActive || filteredClients.some(client => client.groupId === group.id)).map(group => { const groupCollapsed = !searchActive && directoryCollapsedGroups.includes(group.id); const groupSubgroups = clientSubgroups.filter(subgroup => subgroup.groupId === group.id); const visibleSubgroups = groupSubgroups.filter(subgroup => !searchActive || filteredClients.some(client => client.groupId === group.id && client.subgroupId === subgroup.id)); return <section className="directory-group" key={group.id}><button className="directory-group-head" onClick={() => setDirectoryCollapsedGroups(current => groupCollapsed ? current.filter(id => id !== group.id) : [...current, group.id])} aria-expanded={!groupCollapsed}><span className="directory-collapse-icon"><ChevronDown size={15} /></span><strong>{group.name}</strong><span>{filteredClients.filter(c => c.groupId === group.id).length} 位客户</span></button>{!groupCollapsed && visibleSubgroups.map(subgroup => { const subgroupCollapsed = !searchActive && directoryCollapsedSubgroups.includes(subgroup.id); const subgroupClients = sortClientsByNameLength(filteredClients.filter(c => c.subgroupId === subgroup.id)); return <div className="directory-subgroup" key={subgroup.id}><button className="directory-subgroup-head" onClick={() => setDirectoryCollapsedSubgroups(current => subgroupCollapsed ? current.filter(id => id !== subgroup.id) : [...current, subgroup.id])} aria-expanded={!subgroupCollapsed}><span className="directory-collapse-icon"><ChevronDown size={14} /></span><strong>{subgroup.name}</strong><span>{subgroupClients.length} 位</span></button>{!subgroupCollapsed && <div className="directory-client-grid">{subgroupClients.map(c => <div className="directory-client" key={c.id}><button className="directory-client-main" onClick={() => { setSelectedId(c.id); setActive('clients'); }}><span className="client-avatar">{c.name.slice(0, 1)}</span><span><strong>{c.company}</strong><small>{c.name} · {c.phone}</small></span></button><div className="directory-client-actions"><button className="icon-btn" onClick={() => { setSelectedId(c.id); setActive('clients'); }} aria-label={`查看${c.name}`} title="查看客户详情"><ChevronRight size={15} /></button><button className="icon-btn" onClick={() => openClient(c)} disabled={!canEditClient} aria-label={`编辑${c.name}`} title="编辑客户"><Edit3 size={15} /></button><button className="icon-btn danger" onClick={() => removeClient(c.id)} disabled={!canDeleteClient} aria-label={`删除${c.name}`} title="删除客户"><Trash2 size={15} /></button></div></div>)}{!subgroupClients.length && <div className="sub-empty">该二级分类暂无客户</div>}</div>}</div>; })}{!visibleSubgroups.length && <div className="sub-empty">该分组暂无匹配分类</div>}</section>; })}{!filteredClients.length && <div className="empty">暂无匹配客户</div>}</div>}
        {selected && <div className="panel detail">
           <div className="detail-head"><div><div className="detail-avatar">{selected.name.slice(0, 1)}</div><div><h3>{selected.company}</h3><span>{selected.name} · 添加于 {selected.createdAt}</span></div></div><div className="detail-actions"><button className="secondary-btn small" onClick={() => setSelectedId('')} aria-label="返回客户列表" title="返回客户列表"><ArrowLeft size={15} />返回客户列表</button></div></div>
          <div className="contact-strip"><span><UserRound size={14} />{selected.name}</span><span>{selected.phone}</span><span>微信号：{selected.email || '未设置'}</span></div>
    ...
    {dailyExpenseModal && <Modal title={dailyExpenseModal === 'new' ? '添加日常费用' : '编辑日常费用'} onClose={closeDailyExpenseModal}>
      <div className="form-grid daily-expense-form">
        <label>费用单据ID<input value={dailyExpenseModal === 'new' ? makeDailyExpenseDocId(formDailyExpense.recordDate, dailyExpenses) : dailyExpenseModal.docId} readOnly /></label>
        <label>时间选择<input type="date" value={formDailyExpense.recordDate} onChange={event => setFormDailyExpense(current => ({ ...current, recordDate: event.target.value }))} /></label>
        <label>费用类型<select required value={formDailyExpense.expenseType} onChange={event => setFormDailyExpense(current => ({ ...current, expenseType: event.target.value }))}><option value="">请选择费用类型</option>{dailyExpenseTypes.map(item => <option key={item} value={item}>{item}</option>)}</select></label>
    ...
    {userModal && <Modal title={`${userModal === 'new' ? '添加' : '编辑'}团队成员`} onClose={() => setUserModal(null)}><div className="form-grid"><label>姓名<input value={formUser.name} onChange={e => setFormUser({ ...formUser, name: e.target.value })} /></label><label>用户名<input value={formUser.username} onChange={e => setFormUser({ ...formUser, username: e.target.value })} /></label><label>邮箱<input type="email" value={formUser.email} onChange={e => setFormUser({ ...formUser, email: e.target.value })} /></label><label>手机号<input value={formUser.phone} onChange={e => setFormUser({ ...formUser, phone: e.target.value })} /></label><label>角色<select value={formUser.role} onChange={e => setFormUser({ ...formUser, role: e.target.value as Role })}>{permissionGroups.map(group => <option key={group.id} value={group.name}>{group.name}</option>)}</select></label>{userModal === 'new' && <label className="password-change-option"><span>首次登录时要求修改密码</span><input type="checkbox" checked={mustChangeOnFirstLogin} onChange={e => setMustChangeOnFirstLogin(e.target.checked)} /><i /></label>}</div><div className="modal-actions"><button className="secondary-btn" onClick={() => setUserModal(null)}>取消</button><button className="primary-btn" onClick={saveUser}><Check size={16} />{userModal === 'new' ? '添加成员' : '保存修改'}</button></div></Modal>}
    {passwordModal && session && <Modal title={session.mustChangePassword ? '首次登录修改密码' : '修改密码'} onClose={() => { if (!session.mustChangePassword) setPasswordModal(false); }}>{session.mustChangePassword && <div className="permission-note"><ShieldCheck size={18} /><div><strong>请先设置新密码</strong><span>首次登录或管理员重置密码后，需要修改密码才能继续使用。</span></div></div>}<div className="form-grid"><label>新密码<input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="至少8位，至少三类字符" /></label><label>确认新密码<input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} /></label></div><div className="modal-actions"><button className="primary-btn" onClick={saveOwnPassword}><Check size={16} />保存新密码</button></div></Modal>}
  </div>;
}
